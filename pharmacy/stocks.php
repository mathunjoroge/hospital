...
<?php
        $result = $db->prepare("SELECT drug_id,price AS buying_price,  generic_name, brand_name,price*pharm_qty*mark_up AS value ,price*mark_up AS price,price*ins_mark_up AS ins_price, quantity,pharm_qty FROM drugs");
  $result->execute();
  for($i=0; $row = $result->fetch(); $i++){     
      ...
      $buying_price= $row['buying_price'];
       $ins_price= round($row['ins_price']);
      $price= round($row['price']);
      $qtyp= $row['pharm_qty'];
      $tqty= $qty+$qtyp;
      $value= $tqty*$buying_price;
         ?>
<tbody>
<tr>
<td><?php echo $drug; ?></td>
