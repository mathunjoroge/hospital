...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_decorations(self, page_count: int) -> None:
        ...
        self.drawString(
            20 * mm,
            4 * mm,
            f"© {datetime.now(timezone.utc).date().year} — All rights reserved",
        )
        self.drawRightString(
            PAGE_WIDTH - 20 * mm, 4 * mm, f"Page {self._pageNumber} of {page_count}"
        )
        self.restoreState()
...
def build_kpi_table(
    styles: dict[str, ParagraphStyle], data: list[tuple[str, str]]
) -> Table:
    ...
    formatted_data = [
        [
            [Paragraph(val, styles["kpi_val"]), Paragraph(lbl, styles["kpi_lbl"])]
            for val, lbl in data
        ]
    ]
...
def build_feature_table(
    ...
    header_col1 = Paragraph(
        "<b>Module</b>",
        ParagraphStyle("TH1", fontSize=9, fontName="Helvetica-Bold", textColor=WHITE),
    )
    header_col2 = Paragraph(
        "<b>Key Capabilities</b>",
        ParagraphStyle("TH2", fontSize=9, fontName="Helvetica-Bold", textColor=WHITE),
    )
    ...
    td_body_style = ParagraphStyle(
        "TDBody", fontSize=8.5, fontName="Helvetica", textColor=SLATE_700, leading=12
    )
...
def build_story(styles: dict[str, ParagraphStyle]) -> list[Flowable]:
    ...
    story.append(
        Paragraph("Hospital Management Information System", styles["cover_sub"])
    )
    story.append(Spacer(1, 4 * mm))
    story.append(
        Paragraph("Product Overview &amp; Capability Reference", styles["cover_sub"])
    )
    story.append(Spacer(1, 14 * mm))
    story.append(
        Paragraph(
            f"Version 1.0  ·  {datetime.now(timezone.utc).date().strftime('%B %Y')}  ·  Confidential",
            styles["cover_date"],
        )
    )
    ...
    story.append(
        Paragraph("3. Administrative &amp; Operational Modules", styles["section_h"])
    )
    story.append(create_hr())
    story.append(
        build_feature_table(
            ...
            [
                ...
                ("Mortuary", "Deceased-patient registration, body release workflow."),
                (
                    "Analytics Dashboard",
                    (
        ...
        build_feature_table(
            ...
            [
                (
                    "Authentication",
                    (
                        "TOTP-based MFA, 5-attempt account lockout (15-minute timeout), "
                ...
                (
                    "Authorisation",
                    (
                        "Role-based access control (RBAC) across 12 roles; "
                ...
                (
                    "Encryption at Rest",
                    (
                        "Fernet AES-128-CBC + HMAC EncryptedString column type "
                ...
                (
                    ...
                    (
                        "PatientConsent model, Subject Access Request JSON export, "
                ...
                (
                    "CSRF Protection",
                    (
                        "Flask-WTF CSRF tokens on all state-changing forms; "
                        "JWT API endpoints explicitly exempted."
                    ),
                ),
                (
                    "Rate Limiting",
                    "Flask-Limiter on login (5/min POST), disabled in test mode.",
                ),
                (
                    "CI Security Gates",
                    (
                        "pip-audit (17 known advisories documented), bandit SAST, "
                ...
                (
                    "Session Security",
                    (
                        "Redis-backed sessions, 30-minute idle timeout, SameSite=Lax, "
    ...
    story.append(
        build_feature_table(
            ...
            [
                (
                    "Backend",
                    "Python 3.12 · Flask 3.x · SQLAlchemy ORM · Flask-Migrate (Alembic)",
                ),
                (
                    "Database",
                    "PostgreSQL 16 (primary) · SQLite (test / offline fallback)",
                ),
                (
                    "Cache / Queue",
                    "Redis 7 (session store + Celery broker) · Celery 5.4 (async tasks)",
                ),
                ("Real-time", "Flask-SocketIO · WebRTC (telemedicine signalling)"),
                ("Auth", "Flask-Login · Flask-JWT-Extended · PyOTP (TOTP MFA)"),
                (
                    "AI / NLP",
                    "NVIDIA NIM API client · local HuggingFace summariser · DrugCentral DDI DB",
                ),
                (
                    "Containers",
                    "Docker + Docker Compose (web, db, redis, celery_worker, nlp services)",
                ),
                (
                    "CI/CD",
                    "GitHub Actions: lint → bandit → pip-audit → pytest (214 tests, 41.6% cov)",
                ),
                (
                    "Offline / PWA",
                    "Service Worker sw.js · Web App Manifest · offline.html fallback",
                ),
            ],
            col_widths=[4.5 * cm, 12.5 * cm],
        )
        ...
        build_feature_table(
            ...
            [
                ...
                (
                    ...
                    "1,200 req/sec sustained · Flask-Limiter lockout verified",
                ),
            ],
        ...
        build_feature_table(
            ...
            [
                (
                    ...
                    "Expand 50-item curated array to full ICD-10-CM FTS5 index via WHO API.",
                ),
                (
                    "KRA eTIMS Middleware",
                    "Implement tax middleware for automated KRA electronic invoicing.",
                ),
                (
                    "Controlled Drugs",
                    "Schedule I/II/III dispensing log with dual-signature authorization.",
                ),
                (
                    "Hardware Interfacing",
                    "ASTM E1381 MLLP listener for lab analyzers and PACS DICOM worklists.",
                ),
                (
                    "Penetration Testing",
                    "Third-party OWASP web application penetration audit prior to go-live.",
                ),
            ],
            col_widths=[5.5 * cm, 11.5 * cm],
        )
    ...
    story.append(
        Paragraph(
            f"Document generated: {datetime.now(timezone.utc).date().strftime('%d %B %Y')}",
            styles["caption"],
        )
    )
