...
<div class="container" id="details">
  <center><caption>physical examination</caption></center>
...
<center><caption>diagnosis and prescriptions</caption></center>
...
$hpi=$row['hpi'];
$pexam=$row['pexam'];
$datep=date("d-m-Y H:i:s A", strtotime($row['date']));     
?>
<tbody>
<tr>
<?php if (!empty($cc)) {
...
<?php 
	   ...
	   if ($useFdaDrugsList == 1) {
			$result = $db->prepare("SELECT prescribed_meds.dispensed AS dispensed,prescribed_meds.date AS date, ActiveIngredient, DrugName,duration,frequency,code,prescribed_meds.id AS id,prescribed_meds.strength AS strength,roa, dispensed_status as status FROM prescribed_meds RIGHT OUTER JOIN meds ON prescribed_meds.drug=meds.id  WHERE patient=:b");
	   }
		else {
			$result = $db->prepare("SELECT prescribed_meds.dispensed AS dispensed,dispensed_status as status,prescribed_meds.date AS date, generic_name AS ActiveIngredient, brand_name AS DrugName,duration,frequency,code,prescribed_meds.id AS id,prescribed_meds.strength AS strength,roa FROM prescribed_meds RIGHT OUTER JOIN drugs as meds ON prescribed_meds.drug=meds.drug_id  WHERE patient=:b");
		}
