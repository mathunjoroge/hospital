...
//update drugs as dispensed
$drug_ids=$_POST['drug_id'];  
$qtys=$_POST['quantity'];
foreach (array_combine($qtys, $drug_ids) as $qty => $drug_id){
$sql = "UPDATE drugs
SET  pharm_qty=pharm_qty-?
WHERE drug_id=?";
$q = $db->prepare($sql);
$q->execute(array($qty,$drug_id));
...
$q = $db->prepare($sql);
$q->execute(array(':a'=>$receipt,':b'=>$id,':c'=> $mode,':d'=> $payer,':e'=> $amount,':h'=> $mark_up,':j'=>$status));
if ($mode==3) {
$insurance=$payer;
# code...
}
...
WHERE opno=?";
$q = $db->prepare($sql);
$q->execute(array($served,$has_bill,$id));
//offset adm date when patient pays
if ($_POST['du']>0) {
...
header("location: receipt.php?search=$id&response=0&cash=$tender&ward=$ward&du=$days&token=$token&receipt=$receipt&insurance=$insurance&mode=$mode"); 
