...
<?php
       ...
       //get sum amount from pharmacy for the user 
        ...
        $result = $db->prepare("SELECT sum(amount) AS pharmacy_cash FROM cash WHERE date(date)>=:a AND date(date)<=:b AND confirmation=:c AND paid=:e");
  ...
  for($i=0; $row = $result->fetch(); $i++){
     ...
     //get sum amount from lab for the user     
        $result = $db->prepare("SELECT sum(amount) AS lab_cash FROM lab_cash WHERE date(date)>=:a AND date(date)<=:b AND confirmation=:c AND paid=:e ");
        $result->bindParam(':a',$date1);
        $result->bindParam(':b',$date2);
        $result->bindParam(':c',$company);
        $result->bindParam(':e',$e);       
  ...
  for($i=0; $row = $result->fetch(); $i++){
    ...
    //get sum amount from fees for the user     
        $result = $db->prepare("SELECT sum(amount) AS totalfees FROM fees_total WHERE date(date)>=:a AND date(date)<=:b AND confirmation=:c AND paid=:e ");
        $result->bindParam(':a',$date1);
        $result->bindParam(':b',$date2);
        $result->bindParam(':c',$company);
        $result->bindParam(':e',$e);       
  ...
  for($i=0; $row = $result->fetch(); $i++){     
    ...
    //get sum amount from pharmacy for the user     
        $result = $db->prepare("SELECT sum(amount) AS clinicfees FROM clinics_total WHERE date(date)>=:a AND date(date)<=:b AND confirmation=:c AND paid=:e ");
        $result->bindParam(':a',$date1);
        $result->bindParam(':b',$date2);
        $result->bindParam(':c',$company);
        $result->bindParam(':e',$e);       
