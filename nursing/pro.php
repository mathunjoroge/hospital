...
<p class="text text-success">charges have been saved</p>
</div>
<?php } ?>
...
<li class="breadcrumb-item active" aria-current="page">professional charges</li>
...
<form action="pro.php?&response=0" method="GET">
  <span><?php
...
<h3>add professional charges</h3>
<form action="save_pro_charge.php" method="POST">        
<table class="table">
<tr>
<th>service</th>
<th>amount</th>
...
<tr>
<?php
$result = $db->prepare("SELECT*  FROM  fees WHERE is_nursing=4");
$result->execute();
for($i=0; $row = $result->fetch(); $i++){
$id = $row['fees_id'];
$fee = $row['fees_name']; 
...
if (isset($_GET['resp'])) {
    ?>
<div class="container-fluid">
<h3>save pro charges</h3>
<form action="save_editedpro_charge.php" method="POST"> 
<select id="professional" name="done_by" class="form-select form-control" required/>
    <option >--select the professional name--</option>
