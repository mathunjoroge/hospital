...
class InsuranceScheme(db.Model):
    ...
    code        = db.Column(db.String(20), unique=True, nullable=False)   # e.g. 'SHA', 'NHIF', 'AAR'
    name        = db.Column(db.String(120), nullable=False)
    scheme_type = db.Column(db.String(30), nullable=False, default='public')  # public / private / capitation
    ...
    members     = db.relationship('PatientInsurance', back_populates='scheme',
                                  lazy='dynamic', cascade='all, delete-orphan')
    claims      = db.relationship('Claim', back_populates='scheme',
                                  lazy='dynamic')
...
class PatientInsurance(db.Model):
    ...
    scheme_id        = db.Column(db.Integer, db.ForeignKey('insurance_schemes.id'),
                                 nullable=False, index=True)

    member_number    = db.Column(db.String(60), nullable=False)
    principal_member = db.Column(db.String(100), nullable=True)  # Head of household (if dependant)
    relationship     = db.Column(db.String(30), nullable=True)   # self / spouse / child / parent

    start_date       = db.Column(db.Date, nullable=True)
    end_date         = db.Column(db.Date, nullable=True)         # Null = still active
    is_active        = db.Column(db.Boolean, default=True, nullable=False)

    verified_at      = db.Column(db.DateTime, nullable=True)     # When eligibility was last confirmed
    ...
    patient = db.relationship('Patient', backref=db.backref('insurance_memberships', lazy='dynamic'))
...
class Claim(db.Model):
    ...
    # Scheme references
    scheme_claim_ref   = db.Column(db.String(100), nullable=True)  # Reference returned by payer
