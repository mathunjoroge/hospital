...
class MasterClinicalRegimen(db.Model):
    ...
    program_domain = db.Column(
        db.String(20), nullable=False, index=True
    )  # HIV, TB, TPT
    nascop_ntldp_code = db.Column(
        db.String(30), unique=True, nullable=False, index=True
    )  # e.g., AF1A, DR-BPaLM, TPT-3HP
    regimen_acronym = db.Column(db.String(50), nullable=False)  # e.g., TLD, BPaLM, 3HP
    line_tier = db.Column(
        db.String(30), nullable=False
    )  # 1st_Line, 2nd_Line, 3rd_Line_Salvage, TPT
    target_population = db.Column(
        db.String(100), nullable=False
    )  # e.g., Adults ≥30kg, Children <15yrs
