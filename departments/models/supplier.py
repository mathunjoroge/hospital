...
class SupplierReturn(db.Model):
    ...
    status = db.Column(db.String(20), default='DRAFT', nullable=False)  # DRAFT, DISPATCHED, CREDIT_ISSUED
    reason = db.Column(db.String(255), nullable=True)  # Damaged, Expired, Wrong Specification
    ...
    supplier = db.relationship('Supplier', backref=db.backref('returns', lazy='dynamic'))
    purchase_order = db.relationship('PurchaseOrder', backref=db.backref('returns', lazy='dynamic'))
    creator = db.relationship('User', foreign_keys=[created_by_id])
    items = db.relationship('SupplierReturnItem', backref='supplier_return', cascade='all, delete-orphan')
...
class SupplierReturnItem(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    supplier_return_id = db.Column(db.Integer, db.ForeignKey('supplier_returns.id'), nullable=False, index=True)
    item_type = db.Column(db.String(20), default='DRUG', nullable=False)  # DRUG | NON_PHARM
    drug_id = db.Column(db.Integer, db.ForeignKey('drugs.id'), nullable=True, index=True)
    non_pharm_item_id = db.Column(db.Integer, db.ForeignKey('non_pharm_items.id'), nullable=True, index=True)
...
class StockDisposal(db.Model):
    ...
    status = db.Column(db.String(20), default='DRAFT', nullable=False)  # DRAFT, APPROVED, DISPOSED
    ...
    items = db.relationship('StockDisposalItem', backref='disposal', cascade='all, delete-orphan')
...
class StockDisposalItem(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    disposal_id = db.Column(db.Integer, db.ForeignKey('stock_disposals.id'), nullable=False, index=True)
    item_type = db.Column(db.String(20), default='DRUG', nullable=False)  # DRUG | NON_PHARM
    drug_id = db.Column(db.Integer, db.ForeignKey('drugs.id'), nullable=True, index=True)
    non_pharm_item_id = db.Column(db.Integer, db.ForeignKey('non_pharm_items.id'), nullable=True, index=True)
