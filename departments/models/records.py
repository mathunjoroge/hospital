...
class Patient(db.Model):
    ...
    patient_id = db.Column(db.String(10), unique=True, nullable=False, index=True)  # Autogenerated unique patient number
    name = db.Column(db.String(100), nullable=False, index=True)  # e.g., 'John Doe'
    place_of_residence = db.Column(db.String(100), nullable=False)  # e.g., '123 Main St, City'
    ...
    def generate_patient_id(self):
        ...
        with db.session.no_autoflush:  # Prevent premature flush
            last_patient = Patient.query.order_by(Patient.id.desc()).with_for_update().first()
            if last_patient and last_patient.patient_id.startswith(prefix):
                last_number = int(last_patient.patient_id[1:]) if last_patient.patient_id[1:].isdigit() else 0
            else:
                last_number = 0
