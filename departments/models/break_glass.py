...
class BreakGlassAccessLog(db.Model):
    ...
    patient_id = db.Column(db.String(50), nullable=True, index=True)   # Target patient (if applicable)
    resource_type = db.Column(db.String(64), nullable=True)            # e.g. 'Patient', 'LabResult'
    ...
    expires_at = db.Column(db.DateTime(timezone=True), nullable=False)   # invoked_at + duration
    is_active = db.Column(db.Boolean, default=True, nullable=False)      # False once expired/revoked
    ...
    # Relationships
    user = db.relationship('User', backref=db.backref('break_glass_logs', lazy='dynamic'))
