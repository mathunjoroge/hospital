...
class Drug(db.Model):
    ...
    category_id = db.Column(
        db.Integer, db.ForeignKey("drugs_category.id"), nullable=False
    )
    ...
    storage_condition = db.Column(
        db.String(50), default="Ambient", nullable=False
    )  # Ambient | Cold Chain (2-8°C) | Freezer (-20°C)
...
class Purchase(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    drug_id = db.Column(
        db.Integer, db.ForeignKey("drugs.id"), nullable=False
    )  # Links to Drug
    batch_id = db.Column(
...
class DispensedDrug(db.Model):
    ...
    batch_id = db.Column(
        db.Integer, db.ForeignKey("batches.id"), nullable=False
    )  # Store batch_id instead of batch_no
    patient_id = db.Column(
        db.String(50), db.ForeignKey("patients.patient_id"), nullable=False
    )
...
class DrugRequest(db.Model):
    ...
    )  # e.g., Pending, Approved, Denied, Fulfilled
...
class RequestItem(db.Model):
    ...
    request_id = db.Column(
        db.Integer, db.ForeignKey("drug_requests.id"), nullable=False
    )
    drug_id = db.Column(db.Integer, db.ForeignKey("drugs.id"), nullable=False)
