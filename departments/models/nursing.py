...
class NursingNote(db.Model):
    ...
    )  # JSON or delimited string: "Aspirin|81mg|PO|08:00"
...
class Partogram(db.Model):
    ...
    )  # To group entries for the same patient
    timestamp = db.Column(
    ...
    amniotic_fluid = db.Column(
        db.String(20), nullable=False
    )  # e.g., 'clear', 'meconium', 'blood', 'absent'
    moulding = db.Column(
...
class TriageAssessment(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    patient_id = db.Column(
        db.String(50), db.ForeignKey("patients.patient_id"), nullable=False, index=True
    )
    nurse_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=False)
    ...
    patient = db.relationship(
        "Patient", backref=db.backref("triage_history", lazy="dynamic")
    )
