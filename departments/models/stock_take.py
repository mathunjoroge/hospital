...
class StockTake(db.Model):
    ...
    status = db.Column(db.String(20), nullable=False, default='COMPLETED')  # COMPLETED / DRAFT
...
class StockTakeItem(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    stock_take_id = db.Column(db.Integer, db.ForeignKey('stock_takes.id'), nullable=False, index=True)
    item_type = db.Column(db.String(20), nullable=False, default='DRUG')  # DRUG / NON_PHARM
    ...
    reason = db.Column(db.String(255), nullable=True)  # Shrinkage, Damaged, Miscount, Spoilage
