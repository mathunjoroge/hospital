...
class SOAPNote(db.Model):
    ...
    )  # Link to a patient model if available
...
class RequestedLab(db.Model):
    ...
    patient = db.relationship(
        "Patient", backref=db.backref("requested_labs", lazy=True)
    )
    lab_test = db.relationship(
        "LabTest", backref=db.backref("requested_labs", lazy=True)
    )
...
class RequestedImage(db.Model):
    ...
    result_id = db.Column(
        db.String,
        db.ForeignKey("imaging_results.result_id"),
        unique=True,
        nullable=True,
    )
...
class UnmatchedImagingRequest(db.Model):
    ...
    patient = db.relationship(
        "Patient", backref=db.backref("unmatched_imaging_requests", lazy=True)
    )
...
class TheatreList(db.Model):
    ...
    procedure_id = db.Column(
        db.Integer, db.ForeignKey("theatre_procedures.id"), nullable=False
    )
    status = db.Column(db.Integer, default=0)
    created_by = db.Column(db.Integer)
...
class AdmittedPatient(db.Model):
    ...
    )  # User ID of admitting doctor/staff
...
class WardRound(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    admission_id = db.Column(
        db.Integer, db.ForeignKey("admitted_patients.id"), nullable=False
    )
    doctor_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=False)
    notes = db.Column(db.Text, nullable=False)
    status = db.Column(
        db.String(50), default="Under Treatment"
    )  # Example: "Ready for Discharge", "Critical", etc.
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class Disease(db.Model):
    ...
    symptoms = db.relationship(
        "Symptom", secondary="disease_symptoms", back_populates="diseases"
    )
    management_plan = db.relationship(
        "DiseaseManagementPlan", backref="disease", uselist=False, lazy=True
    )
...
class Symptom(db.Model):
    ...
    diseases = db.relationship(
        "Disease", secondary="disease_symptoms", back_populates="symptoms"
    )
...
class OncoPatient(db.Model):
    ...
    )  # Links to Patient model
    diagnosis = db.Column(
        db.String(200), nullable=False
    )  # e.g., 'Breast Cancer, Stage II'
    ...
    patient = db.relationship(
        "Patient",
        backref=db.backref(
            "oncology_records", lazy="dynamic", cascade="all, delete-orphan"
        ),
    )
...
class OncologyNote(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    patient_id = db.Column(
        db.String(10), db.ForeignKey("patients.patient_id"), nullable=False, index=True
    )
...
class OncoDrugCategory(db.Model):
    ...
    drugs = db.relationship(
        "departments.models.medicine.OncologyDrug", backref="category", lazy="dynamic"
    )
...
class OncologyDrug(db.Model):
    ...
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    regimens = db.relationship(
        "departments.models.medicine.RegimenDrugAssociation",
        backref="drug",
        lazy="dynamic",
    )
    warnings = db.relationship(
        "departments.models.medicine.SpecialWarning", backref="drug", lazy="dynamic"
    )
...
class RegimenCategory(db.Model):
    ...
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    regimens = db.relationship(
        "departments.models.medicine.OncologyRegimen",
        backref="category",
        lazy="dynamic",
    )
...
class RegimenDrugAssociation(db.Model):
    ...
    regimen_id = db.Column(
        db.Integer, db.ForeignKey("oncology_regimens.id"), primary_key=True
    )
    drug_id = db.Column(
        db.Integer, db.ForeignKey("oncology_drugs.id"), primary_key=True
    )
...
class OncoPrescription(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    onco_patient_id = db.Column(
        db.Integer, db.ForeignKey("onco_patients.id"), nullable=False, index=True
    )
    regimen_id = db.Column(
        db.Integer, db.ForeignKey("oncology_regimens.id"), nullable=False, index=True
    )
    ...
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    onco_patient = db.relationship(
        "OncoPatient",
        backref=db.backref(
            "prescriptions", lazy="dynamic", cascade="all, delete-orphan"
        ),
    )
    regimen = db.relationship(
        "OncologyRegimen",
        backref=db.backref(
            "prescriptions", lazy="dynamic", cascade="all, delete-orphan"
        ),
...
class OncoTreatmentRecord(db.Model):
    ...
    prescription_id = db.Column(
        db.Integer, db.ForeignKey("onco_prescriptions.id"), nullable=False, index=True
    )
    treatment_date = db.Column(db.Date, nullable=False)
    cycle_number = db.Column(db.Integer, nullable=False)  # e.g., Cycle 1, Cycle 2
    administered_drugs = db.Column(
        db.Text, nullable=True
    )  # JSON or text list of drugs administered
    notes = db.Column(db.Text, nullable=True)  # Observations or side effects
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    onco_patient = db.relationship(
        "OncoPatient",
        backref=db.backref(
            "treatment_records", lazy="dynamic", cascade="all, delete-orphan"
        ),
    )
...
class OncologyBooking(db.Model):
    ...
    start_time = db.Column(db.DateTime, nullable=True)  # optional slot time (B5)
...
class PrescriptionDrugDetail(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    prescription_id = db.Column(
        db.Integer, db.ForeignKey("onco_prescriptions.id"), nullable=False
    )
    drug_id = db.Column(db.Integer, db.ForeignKey("oncology_drugs.id"), nullable=False)
    ...
    prescription = db.relationship(
        "OncoPrescription",
        backref=db.backref(
            "drug_details", lazy="dynamic", cascade="all, delete-orphan"
        ),
    )
    drug = db.relationship(
        "OncologyDrug", backref=db.backref("prescription_details", lazy="dynamic")
    )
...
class CancerTypeStage(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    cancer_type_id = db.Column(
        db.Integer, db.ForeignKey("cancer_types.id"), nullable=False
    )
    cancer_stage_id = db.Column(
        db.Integer, db.ForeignKey("cancer_stages.id"), nullable=False
    )
...
class CancerDetail(db.Model):
    ...
    cancer_type = db.relationship(
        "CancerType",
        backref=db.backref("details", lazy=True, cascade="all, delete-orphan"),
    )
