...
def patient_to_fhir(patient: Patient) -> dict:
    ...
    if patient.next_of_kin:
        contact_person.append({
            "relationship": [{"text": patient.relationship_with_next_of_kin or "Next of Kin"}],
            "name": {"text": patient.next_of_kin},
            "telecom": [{"system": "phone", "value": patient.next_of_kin_contact}] if patient.next_of_kin_contact else []
        })
    ...
    resource = {
        ...
        "meta": {
            "versionId": "1",
            "lastUpdated": (patient.updated_at or patient.date_registered or datetime.utcnow()).isoformat() if hasattr(patient, 'updated_at') else datetime.utcnow().isoformat()
        }
    }
    return resource
...
def vitals_to_fhir_observations(vitals: Vitals) -> list[dict]:
    ...
    metrics = [
        ...
        ('heart_rate', getattr(vitals, 'pulse', None) or getattr(vitals, 'pulse_rate', None)),
        ('bp_systolic', getattr(vitals, 'blood_pressure_systolic', None) or getattr(vitals, 'bp_systolic', None)),
        ('bp_diastolic', getattr(vitals, 'blood_pressure_diastolic', None) or getattr(vitals, 'bp_diastolic', None)),
        ('spo2', getattr(vitals, 'oxygen_saturation', None) or getattr(vitals, 'spo2', None)),
    ...
    for key, val in metrics:
        if val is not None:
            ...
            obs = {
                ...
                "effectiveDateTime": vitals.timestamp.isoformat() if getattr(vitals, 'timestamp', None) else datetime.utcnow().isoformat(),
                "valueQuantity": {
                    "value": float(val),
                    "unit": loinc["unit"],
...
def search_fhir_observations():
    ...
    for v in vitals_records:
        for obs in vitals_to_fhir_observations(v):
            entries.append({"fullUrl": f"{request.host_url}api/fhir/R4/Observation/{obs['id']}", "resource": obs})
    ...
    for note in notes:
        if note.assessment:
            condition = {
                ...
                "recordedDate": note.created_at.isoformat() if getattr(note, 'created_at', None) else datetime.utcnow().isoformat()
            }
            entries.append({"fullUrl": f"{request.host_url}api/fhir/R4/Condition/{condition['id']}", "resource": condition})
...
def search_fhir_diagnostic_reports():
    ...
    for lab in lab_results:
        report = {
            ...
            "code": {"text": lab.test_name if hasattr(lab, 'test_name') else "Laboratory Test"},
            "subject": {"reference": f"Patient/{patient_id}"},
            "issued": lab.timestamp.isoformat() if getattr(lab, 'timestamp', None) else datetime.utcnow().isoformat(),
            "conclusion": str(getattr(lab, 'result_value', ''))
        }
        entries.append({"fullUrl": f"{request.host_url}api/fhir/R4/DiagnosticReport/{report['id']}", "resource": report})
    ...
    for img in imaging_results:
        report = {
            ...
            "code": {"text": img.imaging_type if hasattr(img, 'imaging_type') else "Radiology Study"},
            "subject": {"reference": f"Patient/{patient_id}"},
            "issued": img.created_at.isoformat() if getattr(img, 'created_at', None) else datetime.utcnow().isoformat(),
            "conclusion": getattr(img, 'ai_impression', '') or getattr(img, 'result_notes', '') or "Imaging Study Completed"
        }
        entries.append({"fullUrl": f"{request.host_url}api/fhir/R4/DiagnosticReport/{report['id']}", "resource": report})
    ...
    for rx in prescriptions:
        med_req = {
            ...
            "medicationCodeableConcept": {
                "text": rx.drug_name if hasattr(rx, 'drug_name') else "Prescribed Medication"
            },
            "subject": {"reference": f"Patient/{patient_id}"},
            "authoredOn": rx.date_prescribed.isoformat() if getattr(rx, 'date_prescribed', None) else datetime.utcnow().isoformat(),
            "dosageInstruction": [{
                "text": f"Dosage: {getattr(rx, 'dosage', 'As directed')}, Duration: {getattr(rx, 'duration', 'N/A')}"
            }]
        }
        entries.append({"fullUrl": f"{request.host_url}api/fhir/R4/MedicationRequest/{med_req['id']}", "resource": med_req})
