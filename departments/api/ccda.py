...
def generate_c_cda_xml(patient: Patient) -> str:
    ...
    ET.SubElement(
        root,
        "id",
        {"root": "2.16.840.1.113883.19.5", "extension": f"CCD-{patient.patient_id}"},
    )
    ET.SubElement(
        root,
        "code",
    ...
    ET.SubElement(
        patient_role,
        "id",
        {"root": "2.16.840.1.113883.19.5", "extension": str(patient.patient_id)},
    )
    if patient.national_id:
        ET.SubElement(
            patient_role,
            "id",
            {"root": "2.16.840.1.113883.4.1", "extension": str(patient.national_id)},
        )
    ...
    gender_code = (
        "M"
        if patient.sex in ["M", "Male"]
        else ("F" if patient.sex in ["F", "Female"] else "UN")
    )
    ET.SubElement(
        patient_elem,
        "administrativeGenderCode",
        {"code": gender_code, "codeSystem": "2.16.840.1.113883.5.1"},
    )
    ...
    if patient.date_of_birth:
        dob_str = (
            ...
            else str(patient.date_of_birth).replace("-", "")
        )
    ...
    ET.SubElement(
        assigned_author,
        "id",
        {"root": "2.16.840.1.113883.19.5", "extension": "HIMS-SYSTEM"},
    )
    ...
    ET.SubElement(
        section_all,
        "templateId",
        {"root": "2.16.840.1.113883.10.20.22.2.6.1", "extension": "2015-08-01"},
    )
    ET.SubElement(
        section_all,
        "code",
    ...
    if hasattr(patient, "allergies") and patient.allergies:
        try:
            items = [
                f"{a.allergen} ({getattr(a, 'reaction', 'Adverse reaction') or 'Adverse reaction'})"
    ...
    ET.SubElement(
        section_med,
        "templateId",
        {"root": "2.16.840.1.113883.10.20.22.2.1.1", "extension": "2015-08-01"},
    )
    ET.SubElement(
        section_med,
        "code",
    ...
    ET.SubElement(
        section_prob,
        "templateId",
        {"root": "2.16.840.1.113883.10.20.22.2.5.1", "extension": "2015-08-01"},
    )
    ET.SubElement(
        section_prob,
        "code",
    ...
    ET.SubElement(
        section_vit,
        "templateId",
        {"root": "2.16.840.1.113883.10.20.22.2.4.1", "extension": "2015-08-01"},
    )
    ET.SubElement(
        section_vit,
        "code",
    ...
    vitals_records = (
        Vitals.query.filter_by(patient_id=patient.patient_id)
    ...
    if vitals_records:
        ...
        for v in vitals_records:
            ...
            ET.SubElement(tr, "td").text = str(
                getattr(v, "pulse", "-") or getattr(v, "pulse_rate", "-")
            )
            sys = getattr(v, "blood_pressure_systolic", None) or getattr(
                v, "bp_systolic", "-"
            )
            dia = getattr(v, "blood_pressure_diastolic", None) or getattr(
                v, "bp_diastolic", "-"
            )
            ET.SubElement(tr, "td").text = f"{sys}/{dia}"
            ET.SubElement(tr, "td").text = str(
                getattr(v, "oxygen_saturation", "-") or getattr(v, "spo2", "-")
            )
    else:
    ...
    ET.SubElement(
        section_res,
        "templateId",
        {"root": "2.16.840.1.113883.10.20.22.2.3.1", "extension": "2015-08-01"},
    )
    ET.SubElement(
        section_res,
        "code",
