...
def aggregate_monthly_khis_data(year: int, month: int) -> dict:
    ...
    new_patients_count = Patient.query.filter(
        Patient.date_registered >= start_date,
        Patient.date_registered <= end_date
    ).count()
    ...
    soap_notes = SOAPNote.query.filter(
        SOAPNote.created_at >= start_date,
        SOAPNote.created_at <= end_date
    ).all()
    ...
    lab_tests_count = LabResult.query.filter(
        LabResult.test_date >= start_date,
        LabResult.test_date <= end_date
    ).count()
    ...
    data_elements = [
        {"dataElement": "MOH705A_UNDER5_OPD_MALE", "category": "MOH 705A", "value": opd_under_5_male},
        {"dataElement": "MOH705A_UNDER5_OPD_FEMALE", "category": "MOH 705A", "value": opd_under_5_female},
        {"dataElement": "MOH705B_OVER5_OPD_MALE", "category": "MOH 705B", "value": opd_over_5_male},
        {"dataElement": "MOH705B_OVER5_OPD_FEMALE", "category": "MOH 705B", "value": opd_over_5_female},
        {"dataElement": "MOH711_TOTAL_NEW_PATIENTS", "category": "MOH 711", "value": new_patients_count},
        {"dataElement": "MOH711_TOTAL_ADMISSIONS", "category": "MOH 711", "value": admissions_count},
        {"dataElement": "MOH711_TOTAL_DISCHARGES", "category": "MOH 711", "value": discharges_count},
        {"dataElement": "MOH711_LAB_TESTS_CONDUCTED", "category": "MOH 711", "value": lab_tests_count},
        {"dataElement": "MOH711_PRESCRIPTIONS_ISSUED", "category": "MOH 711", "value": prescriptions_count},
    ]
    ...
    return {
        ...
        "top_diagnoses": sorted([{"diagnosis": k, "count": v} for k, v in diagnosis_counts.items()], key=lambda x: x["count"], reverse=True)[:10],
...
def export_dhis2_csv():
    ...
    writer.writerow(["dataElement", "period", "orgUnit", "categoryOptionCombo", "value"])

    for elem in aggregated["data_elements"]:
        writer.writerow([elem["dataElement"], aggregated["period"], aggregated["orgUnit"], "default", elem["value"]])

    response = Response(output.getvalue(), mimetype="text/csv")
    response.headers["Content-Disposition"] = f"attachment; filename=khis_dhis2_export_{aggregated['period']}.csv"
    return response
