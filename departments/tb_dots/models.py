...
class TBEnrollment(db.Model):
    ...
    )  # Unique TB identifier
    hiv_status = db.Column(
        db.String(20), nullable=True
    )  # positive, negative, unknown, refused_test
    art_enrollment_id = db.Column(
        db.String(36), db.ForeignKey("art_enrollments.id"), nullable=True
    )  # Link to ART if co-infected
    tb_classification = db.Column(
        db.String(20), nullable=True
    )  # pulmonary, extrapulmonary
    site_of_disease = db.Column(db.String(100), nullable=True)  # for extrapulmonary
    bacteriological_status = db.Column(
        db.String(20), nullable=True
    )  # confirmed, clinical, etc.
    treatment_start_date = db.Column(db.DateTime(timezone=True), nullable=True)
    current_regimen_id = db.Column(
        db.String(36), db.ForeignKey("tb_regimens.id"), nullable=True, index=True
    )
    facility_enrolled_at = db.Column(db.String(100), nullable=True)
    recommended_regimen_code = db.Column(
        db.String(30), nullable=True
    )  # DS-TB-ADULT, DR-BPaLM, DR-BPaL, DR-INDIVIDUALIZED
    ...
    doses_taken = db.relationship(
        "DoseTaken", back_populates="tb_enrollment", lazy="dynamic"
    )
    sputum_results = db.relationship(
        "SputumResult", back_populates="tb_enrollment", lazy="dynamic"
    )
    chest_xrays = db.relationship(
        "ChestXRay", back_populates="tb_enrollment", lazy="dynamic"
    )
    hiv_status_records = db.relationship(
        "HIVStatus", back_populates="tb_enrollment", lazy="dynamic"
    )
...
class TBRegimen(db.Model):
    ...
    )  # JSON or comma-separated list of TB drugs
    duration_months = db.Column(
        db.Integer, nullable=False
    )  # Standard duration for this regimen
...
class DoseTaken(db.Model):
    ...
    tb_enrollment_id = db.Column(
        db.String(36), db.ForeignKey("tb_enrollments.id"), nullable=False, index=True
    )
    dose_number = db.Column(db.Integer, nullable=True)  # Which dose in the sequence
    taken_as_directly_observed = db.Column(
    ...
    # Relationship
    tb_enrollment = db.relationship(
        "TBEnrollment", foreign_keys=[tb_enrollment_id], back_populates="doses_taken"
    )
...
class SputumResult(db.Model):
    ...
    smear_result = db.Column(
        db.String(20), nullable=True
    )  # negative, scanty, 1+, 2+, 3+
    culture_result = db.Column(
        db.String(20), nullable=True
    )  # negative, positive, contaminated
    culture_species = db.Column(db.String(50), nullable=True)  # M. tuberculosis, etc.
    drug_susceptibility = db.Column(
        db.Text, nullable=True
    )  # JSON or text for DST results
    # GeneXpert MTB/RIF and DST fields
    genexpert_mtb_result = db.Column(
        db.String(30), nullable=True
    )  # MTB_DETECTED, MTB_NOT_DETECTED, INVALID
    rifampicin_resistance = db.Column(
        db.String(30), nullable=True
    )  # RIF_SUSCEPTIBLE, RIF_RESISTANT, INDETERMINATE
    fluoroquinolone_resistance = db.Column(
        db.String(30), nullable=True
    )  # FQ_SUSCEPTIBLE, FQ_RESISTANT, NOT_TESTED
    ...
    # Relationship
    tb_enrollment = db.relationship(
        "TBEnrollment", foreign_keys=[tb_enrollment_id], back_populates="sputum_results"
    )
...
class ChestXRay(db.Model):
    ...
    # Relationship
    tb_enrollment = db.relationship(
        "TBEnrollment", foreign_keys=[tb_enrollment_id], back_populates="chest_xrays"
    )
...
class HIVStatus(db.Model):
    ...
    tb_enrollment = db.relationship(
        "TBEnrollment",
        foreign_keys=[tb_enrollment_id],
        back_populates="hiv_status_records",
    )
