...
def chatbot_interface():
    ...
    logger.debug(
        f"Session ID: {session_id}, Current conversation length: {len(current_history)}"
    )
        ...
        try:
            ...
            if file and file.filename:
                ...
                if file_size > MAX_FILE_SIZE:
                    logger.warning(
                        f"File too large: {file.filename}, size: {file_size} bytes"
                    )
                    return Response(
                        Summarizer._format_output(
                ...
                logger.debug(
                    f"Extracted content from file {file_name}: {file_content[:80]}..."
                )
            ...
            task = process_clinical_chatbot_task.delay(
                combined_input, conversation_context, patient_id
            )
            logger.info(
                f"Dispatched chatbot task {task.id} via Celery for session {session_id}"
            )
        ...
        conversation=[
            {
                ...
                "response": Summarizer._format_output(entry["content"]),
            }
...
def chatbot_task_status(task_id):
    ...
    if state in ("PENDING", "RECEIVED", "STARTED"):
        return jsonify(
            {"status": "PROCESSING", "state": state, "task_id": task_id}
        ), 200
    elif state == "SUCCESS":
        ...
        if "conversation" in session:
            ...
            if input_text and not any(
                turn.get("content") == input_text for turn in conv
            ):
        ...
        return jsonify(
            {
                ...
                "task_id": task_id,
                "error": str(
                    task_result.info if "task_result" in locals() else "Task failed"
                ),
            }
        ), 500
