    ...
    try:
        ...
        active_enc = (
            Encounter.query.filter_by(
                patient_id=admission.patient_id, status="ACTIVE", encounter_type="IPD"
            )
...
def create_surgical_encounter(
    patient_id: str,
    provider_id: str | None = None,
    chief_complaint: str = "Surgical Procedure",
):
...
def api_adt_admit_a01():
    ...
    if not (patient_id and ward_id and room_id and bed_id):
        return jsonify(
            {
                "status": "error",
                "message": "Missing required fields (patient_id, ward_id, room_id, bed_id).",
            }
        ), 400
...
def api_adt_transfer_a02():
    ...
    if not (admission_id and new_ward_id and new_room_id and new_bed_id):
        return jsonify(
            {
                "status": "error",
                "message": "Missing required fields (admission_id, new_ward_id, new_room_id, new_bed_id).",
            }
        ), 400
...
def api_adt_discharge_a03():
    ...
    if not admission_id:
        return jsonify(
            {"status": "error", "message": "Missing required field 'admission_id'."}
        ), 400
