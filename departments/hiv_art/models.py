...
class ARTEnrollment(db.Model):
    ...
    current_regimen_id = db.Column(
        db.String(36), db.ForeignKey("art_regimens.id"), nullable=True, index=True
    )
    ...
    hei_infant_prophylaxis = db.Column(
        db.String(50), nullable=True
    )  # NVP_SYRUP, AZT_SYRUP, DUAL_PROPHYLAXIS
    eid_dna_pcr_6wk_result = db.Column(
    ...
    adherence_visits = db.relationship(
        "AdherenceVisit", back_populates="enrollment", lazy="dynamic"
    )
    viral_loads = db.relationship(
        "ViralLoad", back_populates="enrollment", lazy="dynamic"
    )
    cd4_counts = db.relationship(
        "CD4Count", back_populates="enrollment", lazy="dynamic"
    )
    who_stages = db.relationship(
        "WHOStage", back_populates="enrollment", lazy="dynamic"
    )
...
class ARTRegimen(db.Model):
    ...
    )  # JSON or comma-separated list of ARVs
...
class AdherenceVisit(db.Model):
    ...
    art_enrollment_id = db.Column(
        db.String(36), db.ForeignKey("art_enrollments.id"), nullable=False, index=True
    )
    ...
    adherence_percentage = db.Column(
        db.Float, nullable=True
    )  # Calculated: (pills_taken/pills_dispensed)*100
    ...
    enrollment = db.relationship(
        "ARTEnrollment",
        foreign_keys=[art_enrollment_id],
        back_populates="adherence_visits",
    )
...
class ViralLoad(db.Model):
    ...
    test_type = db.Column(
        db.String(50), nullable=True
    )  # routine, diagnostic, confirmation
    ...
    # Relationship
    enrollment = db.relationship(
        "ARTEnrollment", foreign_keys=[art_enrollment_id], back_populates="viral_loads"
    )
...
class CD4Count(db.Model):
    ...
    # Relationship
    enrollment = db.relationship(
        "ARTEnrollment", foreign_keys=[art_enrollment_id], back_populates="cd4_counts"
    )
...
class WHOStage(db.Model):
    ...
    defining_conditions = db.Column(
        db.Text, nullable=True
    )  # Conditions that define the stage
    result_date = db.Column(db.Date, nullable=True)
    ...
    # Relationship
    enrollment = db.relationship(
        "ARTEnrollment", foreign_keys=[art_enrollment_id], back_populates="who_stages"
    )
...
class MOH731ARVRegimenPatientMonthly(db.Model):
    ...
    facility_id = db.Column(
        db.String(50), nullable=False, default="KE_MOH_HOSPITAL_001"
    )
    period = db.Column(db.String(6), nullable=False, index=True)  # YYYYMM
    regimen_code = db.Column(
        db.String(20), nullable=False, index=True
    )  # e.g., AF1A, AF1B, PF1A
    regimen_line = db.Column(
        db.String(20), nullable=False
    )  # Adult_1st, Adult_2nd, Ped_1st, Ped_2nd, 3rd_Line
...
class MOH729BARVFCDRRMonthly(db.Model):
    ...
    period = db.Column(db.String(6), nullable=False, index=True)  # YYYYMM
    arv_drug_code = db.Column(
        db.String(40), nullable=False, index=True
    )  # e.g., ARV_TLD_300_300_50
    unit_pack_size = db.Column(
        db.String(20), nullable=False
    )  # Bottle_30s, Bottle_90s, Bottle_180s
