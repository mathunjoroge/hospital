...
def room_ui(session_uuid):
    ...
    user = get_effective_user()
    session = TelemedicineSession.query.filter_by(session_uuid=session_uuid).first_or_404()
...
def save_notes(session_uuid):
    ...
    return jsonify({'message': 'Clinical notes updated', 'notes': session.clinical_notes})
...
def list_sessions():
    ...
    if user.role in ('doctor', 'medicine', 'admin'):
        ...
        query = TelemedicineSession.query.filter_by(patient_id=patient_id) if patient_id else TelemedicineSession.query.filter_by(id=-1)
