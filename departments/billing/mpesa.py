...
def get_mpesa_config():
    ...
    return {
        ...
        'consumer_key': current_app.config.get('MPESA_CONSUMER_KEY', 'test_consumer_key'),
        'consumer_secret': current_app.config.get('MPESA_CONSUMER_SECRET', 'test_consumer_secret'),
        'shortcode': current_app.config.get('MPESA_SHORTCODE', '174379'),
        'passkey': current_app.config.get('MPESA_PASSKEY', 'bfb279f9aa9bdbcf158e97dd71a467cd2e0c893059b10f78e6b72ada1ed2c919'),
        'callback_url': current_app.config.get('MPESA_CALLBACK_URL', 'https://hospital.example.com/api/mpesa/callback'),
    }
...
def initiate_stk_push(phone_number: str, amount: float, account_reference: str, invoice_id: int = None, transaction_desc: str = "Hospital Bill Payment") -> dict:
    """
    ...
    if not token:
        return {"success": False, "error": "Could not authenticate with Safaricom M-Pesa service."}
    ...
    try:
        ...
        if res.status_code == 200 and data.get('ResponseCode') == '0':
            ...
            return {"success": False, "error": data.get('CustomerMessage', 'STK push failed'), **data}
    except Exception as e:
...
def process_mpesa_callback(callback_data: dict) -> dict:
    ...
    try:
        ...
        if result_code == 0:
            ...
            db.session.commit()
            return {"status": "success", "receipt": mpesa_receipt, "amount": paid_amount}
...
def handle_stk_push_route():
    ...
    account_ref = data.get('account_reference', f"INV-{invoice_id}" if invoice_id else "HOSPITAL")
