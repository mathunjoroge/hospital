...
Covers: RequestedLab, RequestedImage, PrescribedMedicine, DispensedDrug,
        TheatreList, AdmittedPatient, ClinicBooking, PaidBill, DrugsBill, Billing
    ...
    try:
        for charge_data in pending:
            try:
                ...
                elif charge_type == 'AdmittedPatient':
                    ward = sync_session.get(Ward, charge_data['ward_id'])
                    if ward:
                        sync_charge(
                            patient_id=charge_data['patient_id'],
                            source_table="admitted_patient",
                            source_id=charge_data['source_id'],
                            description=f"Ward Admission: {ward.name}",
                            category="ward",
                            amount=float(ward.admission_fee or 0),
                            source_encounter_id=charge_data.get('encounter_id'),
                            _session=sync_session,
                        )
                        logger.info(f"Synced ward charge for AdmittedPatient #{charge_data['source_id']}")
