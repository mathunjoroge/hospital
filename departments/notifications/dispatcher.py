...
class AfricasTalkingSMSChannel(BaseChannel):
    ...
    def send(self, log_entry: OutboundNotificationLog) -> bool:
        username = (
            current_app.config.get('AT_USERNAME') or os.environ.get('AT_USERNAME') or 'sandbox'
            if current_app else os.environ.get('AT_USERNAME', 'sandbox')
        )
        api_key = (
            current_app.config.get('AT_API_KEY') or os.environ.get('AT_API_KEY')
            if current_app else os.environ.get('AT_API_KEY')
        )
        sender_id = (
            current_app.config.get('AT_SENDER_ID') or os.environ.get('AT_SENDER_ID')
            if current_app else os.environ.get('AT_SENDER_ID')
        )
        ...
        try:
            ...
            if recipients:
                ...
                if rec_status.lower() in ('success', 'sent', 'queued') or status_code in (100, 101, 102):
                    logger.info(f"[AfricasTalkingSMSChannel] SMS sent to {log_entry.recipient} (msg_id: {rec_msg_id})")
                    return True
                else:
            ...
            logger.info(f"[AfricasTalkingSMSChannel] Response summary: {sms_data.get('Message')}")
            return True
        except Exception as e:
            logger.error(f"[AfricasTalkingSMSChannel] Delivery error to {log_entry.recipient}: {e}")
            raise e
...
def get_sms_channel() -> BaseChannel:
    ...
    sms_driver = (
        current_app.config.get('SMS_CHANNEL') or os.environ.get('SMS_CHANNEL') or 'sandbox'
        if current_app else os.environ.get('SMS_CHANNEL', 'sandbox')
    ).lower()
...
class NotificationDispatcher:
    ...
    def dispatch_event(
        ...
        for ch_name in channels:
            ...
            driver = get_sms_channel() if ch_name == 'sms' else CHANNELS.get(ch_name, CHANNELS['email'])
            try:
