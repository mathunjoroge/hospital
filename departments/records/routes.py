...
def index():
    ...
    return render_template('records/patients_list.html',
                           patients=patients,
                           clinics=clinics,
...
def search_patients():
    ...
    if search_term:
        ...
        patients = []
    results = [{"id": p.patient_id, "text": f"{p.name} ({p.patient_id})"} for p in patients]
...
def new_patient():
    ...
    if request.method == 'POST':
        ...
        if not all([name, place_of_residence, sex, date_of_birth, marital_status,
                    blood_group, contact, next_of_kin, relationship_with_next_of_kin,
                    next_of_kin_contact, national_id, emergency_contact]):
...
def edit_patient(patient_id):
    ...
    if request.method == 'POST':
        ...
        patient.relationship_with_next_of_kin = request.form['relationship_with_next_of_kin']
...
def patient_history(patient_id):
    ...
    prescribed = PrescribedMedicine.query.filter_by(patient_id=patient_id).order_by(PrescribedMedicine.id.desc()).all()

    requested_labs = RequestedLab.query.filter_by(patient_id=patient_id).order_by(RequestedLab.date_requested.desc()).all()

    lab_results = LabResult.query.filter_by(patient_id=patient_id).order_by(LabResult.test_date.desc()).all()

    requested_images = RequestedImage.query.filter_by(patient_id=patient_id).order_by(RequestedImage.date_requested.desc()).all()

    vitals = Vitals.query.filter_by(patient_id=patient_id).order_by(Vitals.timestamp.desc()).all()

    nursing_notes = NursingNote.query.filter_by(patient_id=patient_id).order_by(NursingNote.timestamp.desc()).all()

    admissions = AdmittedPatient.query.filter_by(patient_id=patient_id).order_by(AdmittedPatient.admitted_on.desc()).all()
...
def book_clinic():
    ...
    new_booking = ClinicBooking(patient_id=patient_id, clinic_id=clinic_id, clinic_date=clinic_date)
...
def daily_opd_report():
    ...
    ).group_by(func.date(Patient.date_registered)).order_by(func.date(Patient.date_registered)).all()
    ...
    ).group_by(ClinicBooking.clinic_date).order_by(ClinicBooking.clinic_date).all()

    total_registrations = sum(r.count for r in daily_registrations)
    total_bookings = sum(b.count for b in daily_bookings)

    return render_template('records/reports_daily_opd.html',
...
def clinic_attendance_report():
    ...
    end_str = request.args.get('end')
    try:
        start_date = datetime.strptime(start_str, '%Y-%m-%d').date() if start_str else date.today().replace(day=1)
        end_date = datetime.strptime(end_str, '%Y-%m-%d').date() if end_str else date.today()
    except ValueError:
    ...
    return render_template('records/reports_clinic_attendance.html',
...
def registration_report():
    ...
    monthly_data = db.session.query(
        ...
        extract('year', Patient.date_registered) == year
    ).group_by(extract('month', Patient.date_registered)).order_by(extract('month', Patient.date_registered)).all()
    ...
    sex_data = db.session.query(
        Patient.sex, func.count(Patient.id).label('count')
    ).filter(
        extract('year', Patient.date_registered) == year
    ).group_by(Patient.sex).all()

    available_years = db.session.query(
        extract('year', Patient.date_registered).label('yr')
    ).distinct().order_by(extract('year', Patient.date_registered).desc()).all()
    available_years = [int(r.yr) for r in available_years]

    return render_template('records/reports_registrations.html',
