...
def advance_after_completion(patient_id: str) -> str | None:
    ...
    if pending_labs or pending_imaging:
        new_stage = "AWAITING_RESULTS"
    elif pending_rx:
        new_stage = "AWAITING_PHARMACY"
    else:
        new_stage = "AWAITING_BILLING"
    ...
    if current != new_stage:
        ...
        db.session.commit()
        logger.info(
            "ENCOUNTER STAGE ADVANCED: patient %s -> %s", patient_id, new_stage
        )
        if new_stage == "AWAITING_BILLING":
