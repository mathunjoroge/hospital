...
class DiseaseSymptomMapper:
    ...
    def build_disease_signatures(self) -> Dict[str, set]:
        ...
        try:
            ...
            logger.info(
                f"Built disease signatures for {len(disease_signatures)} diseases"
            )
    ...
    def _process_disease_signature(
        self, disease_id: int, disease_name: str, umls_mapper: UMLSMapper
    ) -> Tuple[str, set]:
        symptoms = set()
        try:
            disease_cui = None
            ...
            with get_sqlite_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT cui FROM disease_keywords WHERE disease_id = ? LIMIT 1",
                    (disease_id,),
                )
                row = cursor.fetchone()
                if row:
