    ...
    def load(cls) -> dict:
        ...
        if cls._config is None:
            cls._config = {
                ...
                "UMLS_DB_URL": os.getenv(
                    "UMLS_DB_URL",
                    "postgresql://postgres:postgres@localhost:5432/hospital_umls",
                ),
                "TRUSTED_SOURCES": os.getenv(
                    "TRUSTED_SOURCES", "MSH,SNOMEDCT_US,ICD10CM,ICD9CM,LNC"
                ).split(","),
                "UMLS_LANGUAGE": os.getenv("UMLS_LANGUAGE", "ENG"),
                "MAX_WORKERS": int(
                    os.getenv("MAX_WORKERS", multiprocessing.cpu_count())
                ),
                "RATE_LIMIT": os.getenv("RATE_LIMIT", "10/minute"),
                "SIMILARITY_THRESHOLD": float(os.getenv("SIMILARITY_THRESHOLD", 1.0)),
                "CANCER_CONFIDENCE_THRESHOLD": float(
                    os.getenv("CANCER_CONFIDENCE_THRESHOLD", 0.3)
                ),
                "AMR_IPC_CONFIDENCE_THRESHOLD": float(
                    os.getenv("AMR_IPC_CONFIDENCE_THRESHOLD", 0.1)
                ),
            }
        return cls._config
