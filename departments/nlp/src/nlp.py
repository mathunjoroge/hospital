...
if spacy:
    try:
        ...
        logger.warning(
            f"Failed to load en_core_sci_sm: {e}. Falling back to en_core_web_sm."
        )
        try:
...
class ClinicalNER:
    ...
    def __init__(self, umls_mapper: UMLSMapper = None):
        ...
        for label, pattern in self.patterns:
            try:
                ...
                logger.error(
                    f"Failed to compile pattern for label {label}: {pattern} ({e})"
                )
        self.temporal_patterns = [
            ...
            (
                re.compile(
                    r"\b(since|for)\s*(\d+\s*(day|days|week|weeks|month|months|year|years))\b",
                    re.IGNORECASE,
                ),
                "DURATION",
            ),
            ...
            (
                re.compile(
                    r"\bfor\s*(three|four|five|six|seven|eight|nine|ten|[1-9]\d*)\s*days?\b",
                    re.IGNORECASE,
                ),
                "DURATION",
            ),
        ...
        self.risk_factor_patterns = [
            ...
            for label, pattern in [
                ...
                (
                    "RISK_FACTOR",
                    r"\b(family history|genetic predisposition|hereditary)\b",
                ),
        ...
        self.terms_regex = (
            re.compile(
                ...
                + "|".join(
                    map(
                        re.escape,
                        sorted(DiseasePredictor.clinical_terms, key=len, reverse=True),
                    )
                )
                + r")\b",
                re.IGNORECASE,
            )
            if DiseasePredictor.clinical_terms
            else None
        )
            ...
            for cancer_type, pattern_list in CANCER_PATTERNS.items():
                for label, pattern in pattern_list:
                    try:
                        ...
                        logger.error(
                            f"Invalid regex pattern for {cancer_type}, label {label}: {pattern} ({e})"
                        )
            return flat
            ...
            with get_sqlite_connection() as conn:
                ...
                for row in cursor.fetchall():
                    try:
                        ...
                        logger.error(
                            f"Invalid regex pattern in database for label {row['label']}: {row['pattern']} ({e})"
                        )
                patterns.extend(_flatten_cancer_patterns())
                logger.info(
                    f"Loaded {len(patterns)} valid patterns from database and cancer patterns"
                )
                return patterns
        except Exception as e:
    ...
    def extract_entities(self, text: str, doc=None) -> List[Tuple[str, str, dict]]:
        ...
        term_matches = (
            {
                match.group().lower()
                for match in self.terms_regex.finditer(text)
                if match.group().lower() not in self.invalid_terms
            }
            if self.terms_regex
            else set()
        )
            ...
            if base_term not in self.invalid_terms:
                if base_term in DiseasePredictor.symptom_cuis:
                    ...
                    with get_sqlite_connection() as conn:
                        cursor = conn.cursor()
                        cursor.execute(
                            "SELECT risk_factor FROM disease_risk_factors WHERE risk_factor = ?",
                            (term,),
                        )
                        if cursor.fetchone():
        ...
        for label, pattern in self.compiled_patterns + self.risk_factor_patterns:
            for match in pattern.finditer(text):
                ...
                if (
                    normalized not in seen_entities
                    and normalized not in self.invalid_terms
                ):
                    context = {
                        "severity": 1.0,
                        "temporal": "UNSPECIFIED",
                    ...
                    if label in ["TUMOR_MARKER", "BLOOD_COUNT", "INFLAMMATORY_MARKER"]:
                        try:
                            ...
                            logger.warning(
                                f"Failed to parse lab result for {match_text}: {e}"
                            )
                    else:
        ...
        symptom_texts = [
            self.umls_mapper.normalize_symptom(ent[0])
            for ent in entities
            if ent[0].lower() not in self.invalid_terms
            and ent[2].get("type") == "SYMPTOM"
        ]
        risk_factor_texts = [
            self.umls_mapper.normalize_symptom(ent[0])
            for ent in entities
            if ent[0].lower() not in self.invalid_terms
            and ent[2].get("type") == "RISK_FACTOR"
        ]

        with ThreadPoolExecutor(
            max_workers=HIMS_CONFIG.get("MAX_WORKERS", 4)
        ) as executor:
            symptom_futures = {
                executor.submit(
                    self._get_diseases_for_symptom, symptom_text
                ): symptom_text
                for symptom_text in set(symptom_texts)
            }
            risk_factor_futures = {
                executor.submit(self._get_diseases_for_risk_factor, rf_text): rf_text
        ...
        for i, (entity_text, entity_label, context) in enumerate(entities):
            ...
            diseases = (
                symptom_disease_map.get(normalized_text, set())
                if context.get("type") == "SYMPTOM"
                else risk_factor_disease_map.get(normalized_text, set())
            )

            if (
                not diseases
                and normalized_text in COMMON_SYMPTOM_DISEASE_MAP
                and context.get("type") == "SYMPTOM"
            ):
            ...
            filtered_diseases = [
                d
                for d in diseases
                if disease_symptom_count.get(d, 0) >= 2 or d.lower() in CANCER_DISEASES
            ]
            if filtered_diseases:
                context["associated_diseases"] = filtered_diseases
                context["disease_symptom_map"] = {
                    d: list(disease_symptom_map[d]) for d in filtered_diseases
                }
                entities[i] = (entity_text, entity_label, context)

        logger.debug(
            f"Entity extraction found {len(entities)} entities in {time.time() - start_time:.3f} seconds"
        )
        return entities
    ...
    def _get_diseases_for_symptom(self, symptom_text: str) -> set:
        try:
            with get_sqlite_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT cui FROM symptoms WHERE name = ?", (symptom_text,)
                )
                row = cursor.fetchone()
    ...
    def _get_diseases_for_risk_factor(self, risk_factor_text: str) -> set:
        try:
            ...
            logger.error(
                f"Error looking up diseases for risk factor '{risk_factor_text}': {e}"
            )
            return set()
    ...
    def extract_keywords_and_cuis(self, note: Dict) -> Tuple[List[str], List[str]]:
        ...
        terms = {
            ent[0].lower()
            for ent in entities
            if ent and ent[0].lower() not in self.invalid_terms
        }
        ...
        logger.debug(
            f"Extracted {len(expected_keywords)} keywords and {len(reference_cuis)} CUIs in {time.time() - start_time:.3f} seconds"
        )
...
class DiseasePredictor:
    ...
    def _load_disease_keywords() -> Dict[str, str]:
            ...
            try:
                if os.path.exists(CANCER_KEYWORDS_FILE):
                    with open(CANCER_KEYWORDS_FILE, "r") as f:
                        ...
                        logger.info(
                            f"Loaded {len(keywords)} cancer keywords from {CANCER_KEYWORDS_FILE}"
                        )
                        return {k.lower(): v for k, v in keywords.items()}
                else:
                    logger.warning(
                        f"External file {CANCER_KEYWORDS_FILE} not found. Using default keywords."
                    )
                    return fallback_disease_keywords
            except Exception as e:
                logger.error(
                    f"Failed to load keywords from {CANCER_KEYWORDS_FILE}: {e}"
                )
                return fallback_disease_keywords
        ...
        try:
            with get_sqlite_connection() as conn:
                ...
                """)
                keywords = {
                    row["keyword"].lower(): row["cui"] for row in cursor.fetchall()
                }
                ...
                try:
                    if os.path.exists(CANCER_KEYWORDS_FILE):
                        with open(CANCER_KEYWORDS_FILE, "r") as f:
                            ...
                            keywords.update(
                                {k.lower(): v for k, v in external_keywords.items()}
                            )
                            logger.info(
                                f"Loaded {len(external_keywords)} additional cancer keywords from {CANCER_KEYWORDS_FILE}"
                            )
                    else:
                        logger.warning(
                            f"External file {CANCER_KEYWORDS_FILE} not found. Skipping external keywords."
                        )
                except Exception as e:
                    logger.error(
                        f"Failed to load external keywords from {CANCER_KEYWORDS_FILE}: {e}"
                    )

                logger.info(
                    f"Loaded {len(keywords)} total disease keywords from database and external file"
                )
                return keywords
        except Exception as e:
    ...
    def _load_management_plans() -> Dict:
        try:
            with get_sqlite_connection() as conn:
                ...
                logger.info(
                    f"Loaded {len(management_plans)} management plans from database"
                )
                return management_plans
        except Exception as e:
            logger.error(f"Failed to load management plans and lab tests: {e}")
            logger.info(
                f"Using {len(fallback_management_plans)} fallback management plans"
            )
    ...
    def _load_risk_factors() -> Dict:
        try:
            with get_sqlite_connection() as conn:
                ...
                logger.info(
                    f"Loaded {sum(len(rf) for rf in risk_factors.values())} risk factors for {len(risk_factors)} diseases"
                )
                return risk_factors
        except Exception as e:
    ...
    def predict_cancer_risk(
        self, text: str, patient_id: str = None
    ) -> Dict[str, float]:
    ...
    def predict_from_text(
        self, text: str, amr_ipc_text: str = None, patient_id: str = None
    ) -> Dict:
        start_time = time.time()
        ...
        for entity in entities:
            ...
            if normalized not in self.ner.invalid_terms:
                if entity[1] == "CLINICAL_TERM" and entity[2].get("type") == "SYMPTOM":
                    ...
                    cancer_relevance_scores[normalized] = entity[2].get(
                        "cancer_relevance", 0.5
                    )
                    ...
                    "BLOOD_COUNT",
                    "INFLAMMATORY_MARKER",
                ] and entity[2].get("abnormal"):
                    lab_abnormalities.add(entity[2]["potential_cancer"])

        logger.debug(
            f"Extracted symptoms: {symptom_terms}, Risk factors: {risk_factor_terms}, Lab abnormalities: {lab_abnormalities}"
        )
        ...
        for disease, signature in self.disease_signatures.items():
            ...
            if disease.lower() in self.risk_factors:
                matches += (
                    len(
                        [
                            rf
                            for rf in risk_factor_terms
                            if any(
                                rf in r["risk_factor"].lower()
                                for r in self.risk_factors[disease.lower()]
                            )
                        ]
                    )
                    * 0.5
            ...
            if matches >= (
                self.min_symptom_count - 1
                if disease.lower() in cancer_diseases
                else self.min_symptom_count
            ):
        ...
        if sorted_diseases:
            primary_threshold = (
                ...
                if any(d["disease"].lower() in cancer_diseases for d in sorted_diseases)
                else self.primary_threshold
            )
            if sorted_diseases[0]["score"] >= primary_threshold:
                primary_diagnosis = sorted_diseases[0]
                differential_diagnoses = (
                    sorted_diseases[1:] if len(sorted_diseases) > 1 else []
                )
            else:
                differential_diagnoses = sorted_diseases

        cancer_probabilities = self.predict_cancer_risk(
            cleaned_text, patient_id=patient_id
        )  # Use cleaned text for cancer model
        max_cancer = max(cancer_probabilities, key=cancer_probabilities.get)
        max_prob = cancer_probabilities[max_cancer]
        if (
            max_prob > HIMS_CONFIG.get("CANCER_CONFIDENCE_THRESHOLD", 0.3)
            and not primary_diagnosis
        ):
        ...
        amr_ipc_probabilities = self.predict_amr_ipc(
            amr_ipc_text if amr_ipc_text else text, patient_id=patient_id
        )  # Use AMR/IPC-specific text if provided
        ...
        logger.info(
            f"Predicted {len(sorted_diseases)} diseases from {len(symptom_terms)} symptoms, {len(risk_factor_terms)} risk factors, and {len(lab_abnormalities)} lab abnormalities in {time.time() - start_time:.3f} seconds"
        )
        return result
    ...
    def process_soap_note(self, note: Dict) -> Dict:
        ...
        try:
            ...
            logger.debug(
                f"Text preparation took {time.time() - start_time:.3f} seconds"
            )
            if not text:
                return {
            ...
            if not amr_ipc_text:
                logger.warning(
                    "No valid text for AMR/IPC prediction from specified fields"
                )
            ...
            terms.update(
                [
                    ...
                    if kw.lower() not in self.ner.invalid_terms
                ]
            )
            ...
            t = time.time()
            predictions = self.predict_from_text(
                text, amr_ipc_text=amr_ipc_text
            )  # Pass AMR/IPC-specific text
            ...
            try:
                ...
                if predictions["primary_diagnosis"]:
                    ...
                    if disease in cancer_diseases or max(
                        predictions["cancer_probabilities"].values()
                    ) > HIMS_CONFIG.get("CANCER_CONFIDENCE_THRESHOLD", 0.3):
                    ...
                    if disease in self.risk_factors:
                        management_plans[disease] = management_plans.get(disease, {})
                        management_plans[disease]["risk_factors"] = self.risk_factors[
                            disease
                        ]
                ...
                for disease in predictions["differential_diagnoses"]:
                    ...
                    if disease_name in self.management_plans:
                        management_plans[disease_name] = self.management_plans[
                            disease_name
                        ]
                    if disease_name in cancer_diseases:
                        management_plans[disease_name] = management_plans.get(
                            disease_name, {}
                        )
                    ...
                    if disease_name in self.risk_factors:
                        ...
                        management_plans[disease_name]["risk_factors"] = (
                            self.risk_factors[disease_name]
                        )

                for lab_abnormality in predictions["lab_abnormalities"]:
                    if lab_abnormality in cancer_diseases:
                        management_plans[lab_abnormality] = management_plans.get(
                            lab_abnormality, {}
                        )
                    ...
                    if lab_abnormality in self.risk_factors:
                        ...
                        management_plans[lab_abnormality]["risk_factors"] = (
                            self.risk_factors[lab_abnormality]
                        )

                amr_ipc_probabilities = predictions["amr_ipc_probabilities"]
                max_amr_ipc_category = max(
                    amr_ipc_probabilities, key=amr_ipc_probabilities.get
                )
                ...
                logger.error(
                    f"Error fetching management plans or AMR/IPC recommendations: {e}"
                )

            logger.debug(
                f"Management plans and AMR/IPC recommendations retrieval took {time.time() - t:.3f} seconds"
            )
            ...
            result = {
                ...
                "amr_ipc_probabilities": predictions["amr_ipc_probabilities"],
                "amr_ipc_recommendations": predictions.get(
                    "amr_ipc_recommendations", {}
                ),
            ...
            logger.info(
                f"Processed note ID {note['id']} in {result['processing_time']:.3f} seconds"
            )
            return result
        except Exception as e:
