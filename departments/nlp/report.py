...
def create_lab_report_pdf(filename="sample_lab_report.pdf"):
    ...
    y_position = draw_text(
        "HMIS Clinical Laboratory", margin, y_position, size=16, bold=True
    )
    y_position = draw_text(
        "123 Medical Way, Health City, HC 12345", margin, y_position, size=10
    )
    y_position = draw_text(
    ...
    # Patient Information
    y_position = draw_text(
        "Patient Information", margin, y_position, size=14, bold=True
    )
    ...
    y_position = draw_text(
        "Complete Blood Count (CBC)", margin, y_position, size=14, bold=True
    )
    y_position = draw_text(
    ...
    for test, result, unit, ref in cbc_tests:
        y_position = draw_text(
            f"{test:<20} {result:<15} {unit:<15} {ref}", margin, y_position, size=10
        )
    y_position -= 10

    # Test Results: Comprehensive Metabolic Panel (CMP)
    y_position = draw_text(
        "Comprehensive Metabolic Panel (CMP)", margin, y_position, size=14, bold=True
    )
    y_position = draw_text(
        "Test Name                Result          Units          Reference Range",
        margin,
