...
class MalariaCase(db.Model):
    ...
    )  # Unique malaria case identifier
    diagnosis_date = db.Column(
        db.DateTime(timezone=True),
        nullable=False,
        default=lambda: datetime.now(timezone.utc),
    )
    malaria_species = db.Column(
        db.String(20), nullable=True
    )  # falciparum, vivax, ovale, malariae, knowlesi, mixed
    parasite_density = db.Column(
        db.Integer, nullable=True
    )  # parasites/µL (if microscopy) or percentage (if RDT)
    ...
    pregnancy_status = db.Column(
        db.String(20), nullable=True
    )  # not_pregnant, pregnant_first_trimester, etc.
    weight_kg = db.Column(
        db.Float, nullable=True
    )  # Patient weight in kg (for MOH-645/743 weight band classification)
    treatment_start_date = db.Column(db.DateTime(timezone=True), nullable=True)
    current_regimen_id = db.Column(
        db.String(36), db.ForeignKey("malaria_regimens.id"), nullable=True, index=True
    )
    ...
    # Relationships
    current_regimen = db.relationship(
        "MalariaRegimen", foreign_keys=[current_regimen_id]
    )
    treatments = db.relationship(
        "MalariaTreatment", back_populates="malaria_case", lazy="dynamic"
    )
    lab_results = db.relationship(
        "MalariaLabResult", back_populates="malaria_case", lazy="dynamic"
    )
...
class MalariaRegimen(db.Model):
    ...
    )  # JSON or comma-separated list of antimalarials
    ...
    restriction_notes = db.Column(
        db.Text, nullable=True
    )  # Contraindications, etc. (e.g., pregnancy, G6PD deficiency)
...
class MalariaTreatment(db.Model):
    ...
    malaria_case_id = db.Column(
        db.String(36), db.ForeignKey("malaria_cases.id"), nullable=False, index=True
    )
    dose_number = db.Column(db.Integer, nullable=True)  # Which dose in the sequence
    administered_as_directly_observed = db.Column(
    ...
    # Relationship
    malaria_case = db.relationship(
        "MalariaCase", foreign_keys=[malaria_case_id], back_populates="treatments"
    )
...
class MalariaLabResult(db.Model):
    ...
    test_type = db.Column(
        db.String(20), nullable=True
    )  # microscopy, RDT, PCR, hemoglobin, etc.
    result_value = db.Column(
        db.String(50), nullable=True
    )  # Result value (e.g., parasite density, hemoglobin level)
    result_interpretation = db.Column(
        db.String(20), nullable=True
    ...
    # Relationship
    malaria_case = db.relationship(
        "MalariaCase", foreign_keys=[malaria_case_id], back_populates="lab_results"
    )
