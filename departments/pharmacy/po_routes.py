...
def manage_suppliers():
    ...
    if request.method == "POST":
        ...
        db.session.commit()
        return jsonify(
            {"message": "Supplier created", "supplier": supplier.to_dict()}
        ), 201
...
def auto_generate_pos():
    ...
    if not supplier:
        return jsonify(
            {"error": "No active supplier found. Please create a supplier first."}
        ), 400
...
def create_manual_po():
    ...
    return jsonify(
        {"message": f"Manual PO {po.po_number} created", "purchase_order": po.to_dict()}
    ), 201
...
def submit_po_order(po_id):
    ...
    for item in po.items:
        if item.vote_head_id:
            ...
            vote_head_totals[item.vote_head_id] = (
...
def receive_po_shipment(po_id):
    ...
    if isinstance(items_input, list):
        for item_data in items_input:
            key = (
                ...
                or item_data.get("drug_id")
                or item_data.get("non_pharm_item_id")
            )
            if key:
    ...
    for po_item in po.items:
        key = (
            po_item.id
            if po_item.id in items_map
            else (po_item.drug_id or po_item.non_pharm_item_id)
        )
    ...
    if (
        po.received_by_id
        and po.approved_by_id
        and po.received_by_id == po.approved_by_id
    ):
        po.sod_warning = True
...
def record_direct_receipt():
    ...
    for item in items_data:
        ...
        unit_cost = float(item.get("unit_cost", 0.0))
        batch_num = (
            item.get("batch_number") or f"B-DIR-{po.id}-{secrets.token_hex(2).upper()}"
        )
        ...
        if not expiry_str:
            return jsonify(
                {"error": "Expiry date (YYYY-MM-DD) is required for all items"}
            ), 400
...
def dispatch_supplier_return(rtv_id):
    ...
    if rtv.status != "DRAFT":
        return jsonify(
            {"error": f"RTV is in {rtv.status} status and cannot be dispatched"}
        ), 400
...
def get_supplier_otif_metrics(supplier_id):
    ...
    received_pos = [
        po for po in pos if po.status == "RECEIVED" and po.ordered_at and po.received_at
    ]
    ...
    avg_actual_lead_time = (
        ...
        if lead_times
        else float(supplier.lead_time_days)
    )
    on_time_rate = (
        round((on_time_count / len(received_pos)) * 100, 1) if received_pos else 100.0
    )
    fill_rate = (
        round((total_received_items / total_ordered_items) * 100, 1)
...
def calculate_smart_reorder():
    ...
    for m in movements:
        ...
        if m.item_type == "DRUG":
            ...
            non_pharm_consumption[m.item_id] = (
                non_pharm_consumption.get(m.item_id, 0) + qty
            )
