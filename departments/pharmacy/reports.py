...
def expiries_report():
    ...
    try:
        ...
        report_data = (
            ...
            .join(Drug, Expiry.drug_id == Drug.id)
            .filter(Expiry.removal_date >= start_date, Expiry.removal_date <= end_date)
            .order_by(Expiry.removal_date.desc())
            .all()
        )
...
def analytics():
    ...
    sales = DrugsBill.query.filter(
        DrugsBill.billed_at.between(start_date, end_date), DrugsBill.status == 1
    ).all()
    sales_data = {}
    for sale in sales:
    ...
    for drug in drugs:
        dispensed_qty = (
            db.session.query(db.func.sum(DispensedDrug.quantity_dispensed))
            .filter(
