...
def dispense_controlled_drug(
    ...
    if new_balance < 0:
        raise ControlledDrugError(
            f"CRITICAL: Dispensing {dose_mg}mg would result in negative balance ({new_balance}mg). Physical count verification required."
        )
...
def submit_shift_reconciliation(
    ...
    if variance != 0:
        raise ControlledDrugError(
            f"DISCREPANCY DETECTED: Variance of {variance}mg. Shift cannot be closed. Supervisor investigation required."
        )
