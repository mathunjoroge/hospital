...
def get_live_queue_rows(provider_id: str):
    ...
    for idx, appt in enumerate(queue, 1):
        if isinstance(appt, dict):
            ...
            patient_display = appt.get("patient_name") or f"Patient {pid}"
            checked_in_time = appt.get("started_at", "--:--")
        ...
        rows.append(
            ...
            f'<td class="px-6 py-4 text-sm text-gray-900 font-semibold">{patient_display} <span class="text-xs text-gray-400">({pid})</span></td>'
            ...
            f'<button hx-post="/appointments/api/call-in/{appt_id}" '
