...
def who_checklist(entry_id):
    ...
    if request.method == "POST":
        ...
        if stage == "sign_in":
            checklist.patient_identity_confirmed = str(
                data.get("patient_identity_confirmed", "false")
            ).lower() in ("true", "1", "on")
            checklist.site_marked = str(data.get("site_marked", "false")).lower() in (
                "true",
                "1",
                "on",
            )
            checklist.anaesthesia_safety_check_completed = str(
                data.get("anaesthesia_safety_check_completed", "false")
            ).lower() in ("true", "1", "on")
            checklist.pulse_oximeter_functioning = str(
                data.get("pulse_oximeter_functioning", "false")
            ).lower() in ("true", "1", "on")
            checklist.known_allergy = str(
                data.get("known_allergy", "false")
            ).lower() in ("true", "1", "on")
            checklist.allergy_details = data.get("allergy_details")
            checklist.difficult_airway_risk = str(
                data.get("difficult_airway_risk", "false")
            ).lower() in ("true", "1", "on")
            checklist.airway_equipment_available = str(
                data.get("airway_equipment_available", "false")
            ).lower() in ("true", "1", "on")
            checklist.blood_loss_risk_over_500ml = str(
                data.get("blood_loss_risk_over_500ml", "false")
            ).lower() in ("true", "1", "on")
            checklist.iv_access_and_fluids_planned = str(
                data.get("iv_access_and_fluids_planned", "false")
            ).lower() in ("true", "1", "on")
            ...
            checklist.team_members_introduced = str(
                data.get("team_members_introduced", "false")
            ).lower() in ("true", "1", "on")
            checklist.verbal_confirm_patient_site_procedure = str(
                data.get("verbal_confirm_patient_site_procedure", "false")
            ).lower() in ("true", "1", "on")
            checklist.critical_steps_surgeon_reviewed = str(
                data.get("critical_steps_surgeon_reviewed", "false")
            ).lower() in ("true", "1", "on")
            checklist.critical_steps_anaesthetist_reviewed = str(
                data.get("critical_steps_anaesthetist_reviewed", "false")
            ).lower() in ("true", "1", "on")
            checklist.critical_steps_nursing_reviewed = str(
                data.get("critical_steps_nursing_reviewed", "false")
            ).lower() in ("true", "1", "on")
            checklist.antibiotic_prophylaxis_given = str(
                data.get("antibiotic_prophylaxis_given", "false")
            ).lower() in ("true", "1", "on")
            checklist.antibiotic_given_within_60min = str(
                data.get("antibiotic_given_within_60min", "false")
            ).lower() in ("true", "1", "on")
            checklist.essential_imaging_displayed = str(
                data.get("essential_imaging_displayed", "false")
            ).lower() in ("true", "1", "on")
            ...
            checklist.instrument_sponge_needle_counts_correct = str(
                data.get("instrument_sponge_needle_counts_correct", "false")
            ).lower() in ("true", "1", "on")
            checklist.specimens_labelled = str(
                data.get("specimens_labelled", "false")
            ).lower() in ("true", "1", "on")
            checklist.equipment_problems_addressed = str(
                data.get("equipment_problems_addressed", "false")
            ).lower() in ("true", "1", "on")
...
def anaesthetic_record(entry_id):
    ...
    if not record:
        record = AnaestheticRecord(
            ...
            patient_id=entry.patient_id,
            encounter_id=entry.encounter_id,
            anaesthetist_id=current_user.id
            if hasattr(current_user, "id") and isinstance(current_user.id, int)
            else None,
        )
    ...
    if request.method == "POST":
        ...
        record.estimated_blood_loss_ml = int(
            data.get("estimated_blood_loss_ml", record.estimated_blood_loss_ml or 0)
        )
        record.crystalloids_ml = int(
            data.get("crystalloids_ml", record.crystalloids_ml or 0)
        )
        record.colloids_ml = int(data.get("colloids_ml", record.colloids_ml or 0))
        record.blood_products_ml = int(
            data.get("blood_products_ml", record.blood_products_ml or 0)
        )
        record.urine_output_ml = int(
            data.get("urine_output_ml", record.urine_output_ml or 0)
        )
...
def postop_note(entry_id):
    ...
    if not note:
        note = PostOpNote(
            ...
            patient_id=entry.patient_id,
            encounter_id=entry.encounter_id,
            surgeon_id=current_user.id
            if hasattr(current_user, "id") and isinstance(current_user.id, int)
            else None,
            preop_diagnosis=entry.procedure.name
            if entry.procedure
            else "Surgical Procedure",
            postop_diagnosis=entry.procedure.name
            if entry.procedure
            else "Surgical Procedure",
            procedure_performed=entry.procedure.name
            if entry.procedure
            else "Surgical Procedure",
    ...
    if request.method == "POST":
        ...
        note.aldrete_activity = int(
            data.get("aldrete_activity", note.aldrete_activity or 2)
        )
        note.aldrete_respiration = int(
            data.get("aldrete_respiration", note.aldrete_respiration or 2)
        )
        note.aldrete_circulation = int(
            data.get("aldrete_circulation", note.aldrete_circulation or 2)
        )
        note.aldrete_consciousness = int(
            data.get("aldrete_consciousness", note.aldrete_consciousness or 2)
        )
...
def instrument_count(entry_id):
    ...
    if not counts:
        counts = SurgicalInstrumentCount(
            theatre_entry_id=entry.id,
            encounter_id=entry.encounter_id,
            scrub_nurse_id=current_user.id
            if hasattr(current_user, "id") and isinstance(current_user.id, int)
            else None,
        )
    ...
    if request.method == "POST":
        ...
        counts.sponges_initial = int(
            data.get("sponges_initial", counts.sponges_initial or 0)
        )
        counts.sponges_added = int(data.get("sponges_added", counts.sponges_added or 0))
        counts.sponges_closing_cavity = int(
            data.get("sponges_closing_cavity", counts.sponges_closing_cavity or 0)
        )
        counts.sponges_closing_skin = int(
            data.get("sponges_closing_skin", counts.sponges_closing_skin or 0)
        )

        counts.needles_initial = int(
            data.get("needles_initial", counts.needles_initial or 0)
        )
        counts.needles_added = int(data.get("needles_added", counts.needles_added or 0))
        counts.needles_closing_cavity = int(
            data.get("needles_closing_cavity", counts.needles_closing_cavity or 0)
        )
        counts.needles_closing_skin = int(
            data.get("needles_closing_skin", counts.needles_closing_skin or 0)
        )

        counts.instruments_initial = int(
            data.get("instruments_initial", counts.instruments_initial or 0)
        )
        counts.instruments_added = int(
            data.get("instruments_added", counts.instruments_added or 0)
        )
        counts.instruments_closing_cavity = int(
            data.get(
                "instruments_closing_cavity", counts.instruments_closing_cavity or 0
            )
        )
        counts.instruments_closing_skin = int(
            data.get("instruments_closing_skin", counts.instruments_closing_skin or 0)
        )
...
def update_or_schedule_api():
    ...
    if start_dt:
        has_conflict, conflict_msg = (
            TheatreOperationsEngine.check_room_schedule_conflict(
                or_room=or_room,
                start_time=start_dt,
