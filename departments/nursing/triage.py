...
def validate_vitals(
    ...
    if hr is not None:
        if hr < (hr_min - 15) or hr > (hr_max + 30):
            ...
            warnings.append(
                f"Abnormal Heart Rate: {hr} bpm (Normal: {hr_min}-{hr_max})"
            )
            if risk_level != "CRITICAL":
                risk_level = "WARNING"
    ...
    if rr is not None:
        if rr < (rr_min - 5) or rr > (rr_max + 15):
            warnings.append(
                f"Extreme Respiratory Rate: {rr} rpm (Normal: {rr_min}-{rr_max})"
            )
            risk_level = "CRITICAL"
        elif rr < rr_min or rr > rr_max:
            warnings.append(
                f"Abnormal Respiratory Rate: {rr} rpm (Normal: {rr_min}-{rr_max})"
            )
            if risk_level != "CRITICAL":
                risk_level = "WARNING"
    ...
    if sbp is not None:
        if sbp < sbp_min or sbp >= 180:
            warnings.append(
                f"Critical Blood Pressure SBP: {sbp} mmHg (Normal: {sbp_min}-{sbp_max})"
            )
            risk_level = "CRITICAL"
        elif sbp > sbp_max:
            warnings.append(
                f"High Blood Pressure SBP: {sbp} mmHg (Normal: {sbp_min}-{sbp_max})"
            )
            if risk_level != "CRITICAL":
                risk_level = "WARNING"
...
def calculate_esi_level(
    ...
    if (
        any(k in complaint_lower for k in life_threatening_keywords)
        or validation["risk_level"] == "CRITICAL"
    ):
        return (
            1,
            "ESI Level 1: Immediate life-saving intervention required (Critical Vitals / Unresponsive).",
        )
    ...
    if (
        any(k in complaint_lower for k in high_risk_keywords)
        or validation["risk_level"] == "WARNING"
    ):
        return (
            2,
            "ESI Level 2: High risk situation, severe distress, or abnormal vital signs.",
        )
    ...
    if resources_needed >= 2:
        return (
            3,
            "ESI Level 3: Urgent — Patient requires 2 or more resources with stable vitals.",
        )
    elif resources_needed == 1:
        return (
            4,
            "ESI Level 4: Less Urgent — Patient requires 1 resource (e.g. Lab or X-Ray only).",
        )
    else:
        return (
            5,
            "ESI Level 5: Non-Urgent — Patient requires no additional diagnostic resources.",
        )
...
def assess_patient_triage():
    ...
    assessment = TriageAssessment(
        ...
        vitals_warning=" | ".join(vitals_val["warnings"])
        if vitals_val["warnings"]
        else None,
...
def get_triage_queue():
    ...
    assessments = (
        TriageAssessment.query.filter_by(priority_status="WAITING")
