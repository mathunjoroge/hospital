    ...
    if request.method == "POST":
        try:
            ...
            medications = request.form.get(
                "medications", ""
            )  # Could be a delimited string or JSON
...
def patient_dashboard(patient_id):
    try:
        ...
        tasks = (
            NursingCareTask.query.filter_by(patient_id=patient_id, status="Pending")
            .order_by(NursingCareTask.created_at.desc())
            .all()
        )
        return render_template(
            "nursing/patient_dashboard.html",
            patient_id=patient_id,
            partogram=partogram,
            notes=notes,
            tasks=tasks,
    ...
    if request.method == "POST":
        try:
            ...
            if not message_text or not doctor_id:
                ...
                return redirect(
                    url_for("nursing.communicate_doctor", patient_id=p_id)
                    if p_id
    ...
    try:
        ...
        if not doctors:
            doctors = User.query.filter(
                User.role.in_(["doctor", "medicine", "admin"])
            ).all()
        return render_template(
...
def get_notifications():
    try:
        notifications = (
            Notifications.query.filter_by(receiver_id=current_user.id, is_read=False)
...
def mark_notification_read(notification_id):
    try:
        notification = Notifications.query.filter_by(
            id=notification_id, receiver_id=current_user.id
        ).first()
        if not notification:
            flash(
