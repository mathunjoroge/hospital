        ...
        return jsonify(
            {
                ...
                "arrival_at": assessment.arrival_at.isoformat()
                if assessment.arrival_at
                else None,
        ...
        return jsonify(
            {
                ...
                "triage_completed_at": assessment.triage_completed_at.isoformat()
                if assessment.triage_completed_at
                else None,
                "re_evaluation_due_at": assessment.re_evaluation_due_at.isoformat()
                if assessment.re_evaluation_due_at
                else None,
            }
        ), 200
    except Exception as e:
...
def assign_ed_bed(assessment_id):
    ...
    try:
        assessment = EDOperationsEngine.assign_bed(
            assessment_id=assessment_id, bed_label=bed_label
        )
        return jsonify(
            {
                "success": True,
                ...
                "bed_assigned_at": assessment.bed_assigned_at.isoformat()
                if assessment.bed_assigned_at
                else None,
            }
        ), 200
    except Exception as e:
        ...
        return jsonify(
            {
                "success": True,
                "assessment_id": assessment.id,
                "seen_by_doctor_at": assessment.seen_by_doctor_at.isoformat()
                if assessment.seen_by_doctor_at
                else None,
                "priority_status": assessment.priority_status,
            }
        ), 200
        ...
        return jsonify(
            {
                ...
                "assessment_id": assessment.id,
                "esi_level": assessment.esi_level,
                "last_re_evaluation_at": assessment.last_re_evaluation_at.isoformat()
                if assessment.last_re_evaluation_at
                else None,
                "re_evaluation_due_at": assessment.re_evaluation_due_at.isoformat()
                if assessment.re_evaluation_due_at
                else None,
...
def discharge_ed_patient(assessment_id):
    ...
    try:
        assessment = EDOperationsEngine.discharge_patient(
            assessment_id=assessment_id, disposition=disposition
        )
        return jsonify(
            {
                "success": True,
                ...
                "disposition_at": assessment.disposition_at.isoformat()
                if assessment.disposition_at
                else None,
            }
        ), 200
    except Exception as e:
