...
def calculate_crcl(
    creatinine_mg_dl: float, age_years: int, weight_kg: float, is_female: bool
) -> float:
    """
    ...
    "metformin": [
        {
            ...
            "severity": "CRITICAL",
            "action": "BLOCK",
            "message": "METFORMIN CONTRAINDICATED: eGFR < 30 mL/min/1.73m². High risk of fatal lactic acidosis.",
        },
        {
            "max_egfr": 45,
            "severity": "HIGH",
            "action": "WARN",
            "message": "RENAL DOSING WARNING: eGFR 30–45 mL/min/1.73m². Reduce maximum dose to 1000 mg/day; monitor eGFR every 3 months.",
        },
    ],
    "ciprofloxacin": [
        {
            ...
            "severity": "HIGH",
            "action": "WARN",
            "message": "RENAL DOSING WARNING: eGFR < 30 mL/min/1.73m². Reduce Ciprofloxacin dose by 50% or extend interval to q24h.",
        },
    ],
    "enoxaparin": [
        {
            ...
            "severity": "CRITICAL",
            "action": "WARN",
            "message": "RENAL DOSING ALERT: Severe renal impairment (eGFR < 30 mL/min). Reduce therapeutic dose to 1 mg/kg ONCE daily (q24h).",
        },
    ],
    "gentamicin": [
        {
            ...
            "severity": "HIGH",
            "action": "WARN",
            "message": "RENAL DOSING ALERT: Severe renal impairment. Therapeutic drug monitoring (TDM) mandatory; extend dosing interval (q48h) or dose reduce.",
        },
    ],
    "vancomycin": [
        {
            ...
            "severity": "HIGH",
            "action": "WARN",
            "message": "RENAL DOSING ALERT: Renal impairment (eGFR < 45 mL/min). Monitor serum trough levels (target 15-20 mcg/mL); adjust dosing interval.",
        },
    ],
    "allopurinol": [
        {
            ...
            "severity": "HIGH",
            "action": "WARN",
            "message": "RENAL DOSING ALERT: eGFR < 30 mL/min. Reduce Allopurinol max dose to 100 mg/day to prevent Steven-Johnson / hypersensitivity syndrome.",
        },
    ],
    "digoxin": [
        {
            ...
            "severity": "HIGH",
            "action": "WARN",
            "message": "RENAL DOSING ALERT: eGFR < 30 mL/min. Reduce Digoxin dose by 50% (0.0625 mg/day or QOD); high risk of fatal digitalis toxicity.",
        },
    ],
    "fluconazole": [
        {
            ...
            "severity": "HIGH",
            "action": "WARN",
            "message": "RENAL DOSING ALERT: eGFR < 50 mL/min. Administer standard loading dose, then reduce maintenance dose by 50%.",
        },
    ],
    "acyclovir": [
        {
            ...
            "severity": "HIGH",
            "action": "WARN",
            "message": "RENAL DOSING ALERT: eGFR < 25 mL/min. Reduce IV Acyclovir dose to 5 mg/kg q24h; ensure adequate hydration to prevent renal precipitation.",
        },
    ],
    "atenolol": [
        {
            ...
            "severity": "HIGH",
            "action": "WARN",
            "message": "RENAL DOSING ALERT: eGFR < 35 mL/min. Reduce Atenolol max dose to 50 mg/day (or 25 mg/day if eGFR < 15).",
        },
    ],
}
...
class HepaticDosingEngine:
    ...
    @staticmethod
    def evaluate(
        drug_name: str, has_hepatic_impairment: bool = False, is_cirrhotic: bool = False
    ) -> list[dict]:
        alerts = []
...
PEDIATRIC_DOSING_RULES = {
    "paracetamol": {
        "mg_per_kg_dose": 15.0,
        "adult_max_dose_mg": 1000.0,
        "max_daily_mg_per_kg": 60.0,
    },
...
class PediatricDosingEngine:
    ...
    @staticmethod
    def evaluate(
        drug_name: str, dose_mg: float | None, weight_kg: float | None, age_years: int
    ) -> list[dict]:
        alerts = []
        d_lower = (drug_name or "").lower().strip()
            ...
            for drug_key, rule in PEDIATRIC_DOSING_RULES.items():
                if drug_key in d_lower:
                    ...
                    if "mg_per_kg_dose" in rule:
                        ...
                        if dose_mg > (
                            capped_rec_dose * 1.25
                        ):  # >25% higher than recommended
                            alerts.append(
                                {
...
class AlertFatigueManager:
    ...
    def process_and_filter(
        ...
        cutoff_time = datetime.now(timezone.utc) - timedelta(
            hours=self.suppression_window_hours
        )
