...
def create_transfer():
    ...
    for item_info in items_data:
        ...
        toi = TransferOrderItem(
            ...
            non_pharm_item_id=item_info.get("non_pharm_item_id")
    ...
    log_audit_event(
        ...
        details={
            "target_facility": target_facility.name,
...
def dispatch_transfer(transfer_id):
    ...
    if transfer.status != "DRAFT":
        return jsonify(
            {
                "error": f"Transfer is in {transfer.status} status and cannot be dispatched"
            }
        ), 400
    ...
    for toi in transfer.items:
        ...
        qty_dispatched = int(
            dispatch_info.get("quantity_dispatched", toi.quantity_requested)
        )

        toi.quantity_dispatched = qty_dispatched
        toi.batch_number = (
            dispatch_info.get("batch_number") or f"B-TR-{transfer.id}-{toi.id}"
        )
...
def receive_transfer(transfer_id):
    ...
    for toi in transfer.items:
        ...
        qty_rcvd = int(
            receive_info.get(
                "quantity_received", toi.quantity_dispatched or toi.quantity_requested
            )
        )
        ...
        if toi.item_type == "DRUG" and toi.drug:
            drug = toi.drug
            batch_num = (
                receive_info.get("batch_number")
                or toi.batch_number
                or f"B-TR-{transfer.id}-{toi.id}"
            )
    ...
    all_fulfilled = all(
        toi.quantity_received >= toi.quantity_dispatched for toi in transfer.items
    )
