...
def index():
    try:
        ...
        pending_requests = (
            DrugRequest.query.filter(DrugRequest.status.in_(["Submitted", "Pending"]))
        ...
        current_app.logger.debug(
            "[%s -> index()] Found %d pending requests",
            FILE_NAME,
...
@login_required
@roles_required(
    "store", "stores", "admin", "imaging", "medicine", "nursing", "pharmacy"
)
def inventory():
    try:
...
def issue_request(request_id):
    try:
        drug_request = (
            ...
            .options(joinedload(DrugRequest.items).joinedload(RequestItem.drug))
        ...
        if request.method == "POST":
            try:
                ...
                for item in drug_request.items:
                    ...
                    if (
                        quantity_issued is None
                        or quantity_issued < 0
                        or quantity_issued > item.quantity_requested
                    ):
                    ...
                    if quantity_issued > 0:
                        expiry_date_str = request.form.get(
                            f"expiry_date_{item.id}"
                        ) or request.form.get("expiry_date")
                        batch_number_str = (
                            request.form.get(f"batch_number_{item.id}")
                            or request.form.get("batch_number")
                            or f"REQ-{drug_request.id}"
                        )

                        if not expiry_date_str:
                            flash(
                                f"Expiry date (YYYY-MM-DD) is required for issued drug {item.drug.generic_name}",
                                "error",
                            )
                            return render_template(
                                "stores/issue_request.html",
                        ...
                        try:
                            ...
                            flash(
                                f"Invalid expiry date format for {item.drug.generic_name}. Use YYYY-MM-DD.",
                                "error",
                            )
                            return render_template(
                                "stores/issue_request.html",
                        ...
                        batch = Batch.query.filter_by(
                            drug_id=item.drug_id, batch_number=batch_number_str
                        ).first()
                        if not batch:
                            batch = Batch(
...
def non_pharms():
    try:
        ...
        items = NonPharmItem.query.order_by(
            NonPharmItem.category_id, NonPharmItem.name
        ).all()
        ...
        current_app.logger.debug(
            "[%s -> non_pharms()] Loaded %d non-pharm items across %d categories",
            FILE_NAME,
            len(items),
            len(categories),
        )
...
def get_bin_card(item_type, item_id):
    ...
    movements = (
        StockMovement.query.filter_by(item_type=item_type_upper, item_id=item_id)
...
def get_reconciliation_report():
    ...
    non_pharm_ids = [
        n.id for n in NonPharmItem.query.with_entities(NonPharmItem.id).all()
    ]

    report = {
        "drugs": [reconcile_stock_balance("DRUG", did) for did in drug_ids],
        "non_pharm": [
            reconcile_stock_balance("NON_PHARM", nid) for nid in non_pharm_ids
        ],
    }
...
@login_required
@roles_required(
    "store", "stores", "pharmacy", "admin", "Storekeeper", "Admin", "Pharmacist"
)
...
def list_suppliers_view():
    ...
    avg_lead_time = (
        (sum(s.lead_time_days for s in suppliers) / len(suppliers)) if suppliers else 0
    )
    return render_template(
...
def list_transfers_view():
    ...
    outward_transfers = [
        t for t in transfers if t.source_facility_id == home_facility.id
    ]
    inward_transfers = [
        t for t in transfers if t.target_facility_id == home_facility.id
    ]
    return render_template(
...
def new_transfer_view():
    ...
    home_facility = get_home_facility()
    target_facilities = Facility.query.filter(
        Facility.id != home_facility.id, Facility.is_active.is_(True)
    ).all()
    drugs = Drug.query.order_by(Drug.generic_name).all()
    non_pharms = NonPharmItem.query.order_by(NonPharmItem.name).all()
    return render_template(
...
def list_disposals_view():
    ...
    non_pharms = NonPharmItem.query.order_by(NonPharmItem.name).all()
    batches = (
        Batch.query.filter(Batch.quantity_in_stock > 0)
...
def create_stock_disposal():
    ...
    for item in items_data:
        ...
        disp_item = StockDisposalItem(
            ...
            non_pharm_item_id=item.get("non_pharm_item_id")
            if item_type == "NON_PHARM"
            else None,
            batch_id=batch_id,
...
def approve_stock_disposal(disposal_id):
    ...
    if disposal.status != "DRAFT":
        return jsonify(
            {
                "error": f"Disposal record is in {disposal.status} status and cannot be re-approved"
            }
        ), 400
    ...
    for item in disposal.items:
        ...
        if item.item_type == "DRUG" and item.drug:
            ...
            if item.batch_id and item.batch:
                item.batch.quantity_in_stock = max(
                    0, item.batch.quantity_in_stock - qty
                )
...
def create_commodity_requisition():
    ...
    if not item_id or quantity <= 0:
        return jsonify(
            {"error": "item_id and positive quantity_requested are required"}
        ), 400
...
def list_stock_takes_view():
    ...
    non_pharms = NonPharmItem.query.order_by(NonPharmItem.name).all()
    return render_template(
        "stores/stock_take.html", takes=takes, drugs=drugs, non_pharms=non_pharms
    )
...
def create_stock_take():
    ...
    for item in items_data:
        ...
        drug_id = item.get("drug_id") if item_type == "DRUG" else None
        non_pharm_id = (
            item.get("non_pharm_item_id") if item_type == "NON_PHARM" else None
        )
