...
<tr>  <th>&nbsp;</th>
 ...
 <tbody>
  <?php     
  $result = $db->prepare("SELECT * FROM visits RIGHT OUTER JOIN patients ON visits.patient=patients.opno  WHERE date(visits.date)>=:a AND date(visits.date)<=:b AND age>=:c");
  $result->bindParam(':a',$a);
  $result->bindParam(':b',$b);
  $result->bindParam(':c',$c);
  $result->execute();
       for($i=0; $row = $result->fetch(); $i++){ 
       $total=count($row['opno'],1);      
