...
class TestAuditAndPWA(unittest.TestCase):
    def setUp(self):
        ...
        if not admin:
            admin = User(
                ...
                password=generate_password_hash('AdminTest123!', method='pbkdf2:sha256'),
                role='admin'
            )
    ...
    def test_admin_audit_trail_route(self):
        ...
        with app.app_context():
            log_audit_event(action='BILL_PAYMENT', resource_type='Invoice', resource_id='INV-100', user_id=1, username='admin')
            log_audit_event(action='PATIENT_CREATE', resource_type='Patient', resource_id='P100', user_id=1, username='admin')
    ...
    def test_export_audit_trail_siem(self):
        ...
        with app.app_context():
            log_audit_event(action='LOGIN_SUCCESS', resource_type='User', resource_id='1', user_id=1, username='admin')
