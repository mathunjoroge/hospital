...
def test_genexpert_dr_tb_decision_tree():
    ...
    ds_adult = classify_dr_tb_regimen_from_genexpert("MTB_DETECTED", "RIF_SUSCEPTIBLE", age_years=30)
    assert ds_adult["recommended_code"] == "DS-TB-ADULT"

    ds_ped = classify_dr_tb_regimen_from_genexpert("MTB_DETECTED", "RIF_SUSCEPTIBLE", age_years=8)
    assert ds_ped["recommended_code"] == "DS-TB-PED"

    # Scenario 3: MDR/RR-TB FQ Susceptible -> BPaLM
    bpalm = classify_dr_tb_regimen_from_genexpert("MTB_DETECTED", "RIF_RESISTANT", "FQ_SUSCEPTIBLE")
    assert bpalm["recommended_code"] == "DR-BPaLM"
    assert bpalm["regimen_acronym"] == "BPaLM"

    # Scenario 4: Pre-XDR TB FQ Resistant -> BPaL
    bpal = classify_dr_tb_regimen_from_genexpert("MTB_DETECTED", "RIF_RESISTANT", "FQ_RESISTANT")
    ...
    indiv = classify_dr_tb_regimen_from_genexpert("MTB_DETECTED", "RIF_RESISTANT", prior_bdq_exposure=True)
...
def test_dhis2_exporter_pmtct_genexpert(app):
    ...
    with app.app_context():
        ...
        data_elements = {elem["dataElement"]: elem["value"] for elem in export["data_elements"]}
