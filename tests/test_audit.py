...
def test_audit_logging_on_patient_crud(app):
    with app.app_context():
        ...
        insert_logs = Log.query.filter(
            Log.message.like("%Audit [INSERT] Patient%")
        ).all()
        assert len(insert_logs) >= 1
        assert (
            "PTEST001" in insert_logs[0].message or "Patient" in insert_logs[0].message
        )
        ...
        update_logs = Log.query.filter(
            Log.message.like("%Audit [UPDATE] Patient%")
        ).all()
        ...
        delete_logs = Log.query.filter(
            Log.message.like("%Audit [DELETE] Patient%")
        ).all()
