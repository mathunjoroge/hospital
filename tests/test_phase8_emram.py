    ...
    with app.app_context():
        ...
        if not patient:
            ...
            allergy = PatientAllergy(
                patient_id=patient.patient_id, allergen="Penicillin", reaction="Hives"
            )
...
def test_medicine(app):
    ...
    with app.app_context():
        ...
        if not med:
            med = Medicine(
                generic_name="Amoxicillin", brand_name="Amoxil", dosage="500mg"
            )
            db.session.add(med)
            db.session.commit()
        yield med
...
class TestBCMAWorkflow:
    ...
    def test_bcma_verify_success(
        self, client, app, nurse_user, test_patient, test_medicine
    ):
        with app.app_context():
        ...
        client.post(
            "/login", data={"username": nurse_user.username, "password": "password123"}
        )
    ...
    def test_bcma_verify_patient_mismatch(self, client, app, nurse_user, test_patient):
        ...
        response = client.post(
            "/nursing/bcma/verify",
            json={"patient_barcode": "PAT:WRONG-PATIENT-ID", "drug_barcode": "123"},
        )
        assert response.status_code == 409
        data = response.get_json()
        assert data["match"] is False

    def test_bcma_administer_with_scan(
        self, client, app, nurse_user, test_patient, test_medicine
    ):
        with app.app_context():
            rx = PrescribedMedicine(
...
class TestETLAdditional:
    ...
    def test_etl_endpoint_exists(self, client, admin_user):
        ...
        assert resp.status_code in [
            ...
            400,
            500,
        ]  # 200 success, 400 bad request, 500 error
