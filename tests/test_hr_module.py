...
def test_employee_profile_shows_leave_history(client, app, linked_pair):
    ...
    client.post(
        "/login",
        data={"username": "linked_nurse", "password": "Nurse!2345"},
...
def test_update_profile_rejects_blank_name(hr_client, app):
    ...
    response = hr_client.post(
        f"/hr/update_profile/{employee.id}",
        data={
            "name": "",
            "department": "nursing",
            "job_group": "Group A",
...
def test_employee_cannot_update_another_employees_profile(client, app):
    ...
    client.post(
        "/login",
        data={"username": "plain_emp", "password": "Plain!234"},
        follow_redirects=True,
    )
    client.post(
        f"/hr/update_profile/{other.id}",
        data={
            "name": "Hacked Name",
            "department": "nursing",
            "job_group": "Group A",
...
def test_linked_employee_can_update_own_profile_via_update_profile(
    client, app, linked_pair
):
    employee, user = linked_pair
    client.post(
        "/login",
...
def test_delete_employee_with_history_is_deactivated_not_deleted(hr_client, app):
    ...
    response = hr_client.post(
        f"/hr/delete_employee/{employee_pk}", follow_redirects=True
    )
    assert response.status_code == 200
...
def test_leave_request_success_redirects_employees_away_from_403(
    client, app, linked_pair
):
    """
...
def hr_client(client, app):
    ...
    client.post(
        "/login",
        data={"username": "hr_clerk", "password": "Clerk!2345"},
        follow_redirects=True,
    )
    return client
...
def test_update_employee_no_longer_silently_fails(hr_client, app):
    ...
    response = hr_client.post(
        f"/hr/update_employee/{employee.id}",
        data={
            "name": "New Name",
            "role": "pharmacy",
            "department": "pharmacy",
...
def test_id_collision_no_longer_grants_cross_access(client, app):
    ...
    client.post(
        "/login",
        data={"username": attacker_username, "password": "Attack!234"},
        follow_redirects=True,
    )
...
def test_unlinked_employee_gets_empty_payslips_not_someone_elses(client, app):
    ...
    client.post(
        "/login",
        data={"username": "unlinked_user", "password": "Unlinked!23"},
        follow_redirects=True,
    )
    response = client.get("/hr/payslips")
    assert response.status_code == 200
...
def test_leave_request_without_link_is_refused_not_miswritten(client, app):
    ...
    client.post(
        "/login",
        data={"username": "no_link_user", "password": "NoLink!234"},
        follow_redirects=True,
    )
    response = client.post(
        "/hr/leave_request",
...
def test_cannot_link_one_account_to_two_employees(hr_client, app):
    ...
    hr_client.post(
        f"/hr/update_employee/{emp_a.id}",
        data={
            "name": emp_a.name,
            "role": emp_a.role,
            "department": emp_a.department,
            "linked_username": "shared_login",
        },
    )
    hr_client.post(
        f"/hr/update_employee/{emp_b.id}",
        data={
            "name": emp_b.name,
            "role": emp_b.role,
            "department": emp_b.department,
            "linked_username": "shared_login",
        },
    )
...
def test_hr_templates_reference_no_missing_endpoints(app):
    ...
    assert not broken, (
        "HR templates reference endpoints that do not exist:\n" + "\n".join(broken)
    )
...
def test_leave_request_sick_leave_is_not_capped_by_vacation_balance(
    client, app, linked_pair
):
    employee, user = linked_pair
